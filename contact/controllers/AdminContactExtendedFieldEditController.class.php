<?php
/*##################################################
 *                       AdminContactExtendedFieldEditController.class.php
 *                            -------------------
 *   begin                : March 1, 2013
...
class AdminContactExtendedFieldEditController extends AdminController
{
	private $tpl;
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		$extended_field_cache = ContactExtendedFieldsCache::load()->get_extended_field($id);
		...
		return new AdminContactDisplayResponse($this->tpl, LangLoader::get_message('contact_config.fields.edit', 'contact_common', 'contact'));
	}
	...
	private function build_form($id)
	{
		...
		$fieldset = new FormFieldsetHTML('edit_fields', LangLoader::get_message('contact_config.fields.edit', 'contact_common', 'contact'));
	...
	private function save($id)
	{
		$extended_field = new ExtendedField();
		$extended_field->set_id($id);
		$extended_field = ContactExtendedFieldsService::data_field($extended_field);
		...
		$extended_field->set_position(PersistenceContext::get_sql()->query("SELECT MAX(position) + 1 FROM " . ContactSetup::$contact_extended_fields_table . "", __LINE__, __FILE__));
		...
		ContactExtendedFieldsService::update($extended_field);
	}
