...
ContactFormFieldSelectMarker.prototype = {
	add_field : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<input/> ', {type : 'number', id : 'field_latitude_' + id, name : 'field_latitude_' + id, class : 'input-location-latlng', pattern : '-?\d{2,16}\.\d+', placeholder : '{@form.latitude}'}).appendTo('.location-datas_' + id);
			jQuery('#' + id).append(' ');

			jQuery('<input/> ', {type : 'number', id : 'field_longitude_' + id, name : 'field_longitude_' + id, class : 'input-location-latlng', pattern : '-?\d{2,16}\.\d+', placeholder : '{@form.longitude}'}).appendTo('.location-datas_' + id);
			jQuery('#' + id).append(' ');
