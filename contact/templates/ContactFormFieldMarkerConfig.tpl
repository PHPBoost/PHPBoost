...
ContactFormFieldSelectMarker.prototype = {
	add_field : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<input/> ', {type : 'text', id : 'field_popup_title_' + id, name : 'field_popup_title_' + id, placeholder : '{@form.popup.title}'}).appendTo('#' + id);
...
<div id="input_fields_${escape(ID)}">
	...
	<div id="${escape(ID)}_{fieldelements.ID}">
		...
		<script>
		jQuery(function(){
			jQuery("#geocomplete_${escape(ID)}_{fieldelements.ID}").geocomplete({
				details: ".map-location_${escape(ID)}_{fieldelements.ID}",
				detailsAttribute: "data-location",
				location: ["{fieldelements.MAP_LATITUDE}", "{fieldelements.MAP_LONGITUDE}"],
