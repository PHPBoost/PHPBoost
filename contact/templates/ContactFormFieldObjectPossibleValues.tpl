...
ContactFormFieldObjectPossibleValues.prototype = {
	add : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<a/>', {href : 'javascript:ContactFormFieldObjectPossibleValues.delete('+ this.integer +');', title : "${LangLoader::get_message('delete', 'common')}"}).html('<i class="far fa-delete"></i>').appendTo('#' + id);
...
<div id="input_fields_${escape(HTML_ID)}">
	...
	<div id="${escape(HTML_ID)}_{fieldelements.ID}">
		...
		<a href="javascript:ContactFormFieldObjectPossibleValues.delete({fieldelements.ID});" title="${LangLoader::get_message('delete', 'common')}" data-confirmation="delete-element"><i class="far fa-delete"></i></a>
		<script>
		<!--
...
<a href="javascript:ContactFormFieldObjectPossibleValues.add();" id="add-${escape(HTML_ID)}" class="form-field-more-values" title="${LangLoader::get_message('add', 'common')}"><i class="fas fa-plus"></i></a>
