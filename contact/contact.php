...
if (!empty($mail_valid))
{
    ...
    if (!$CONFIG_CONTACT['contact_verifcode'] || $captcha->is_valid()) //Code de v�rification si activ�
    {
        import('io/mail');
        $mail = new Mail();

        if ($mail->send_from_properties($CONFIG['mail'], $mail_object, $mail_contents, $mail_from, '', 'user')) //Succ�s mail
        {
            $get_error = 'success';
        }
        else //Erreur mail
...
elseif ($get_error == 'success')//Message de succ�s.
{
    $Errorh->handler($LANG['success_mail'], E_USER_SUCCESS);
}
elseif ($get_error == 'error')//Message de succ�s.
{
