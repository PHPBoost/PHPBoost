	...
	if( !empty($question) )
	{
		...
		{	
			...
			{				
				...
				$votes .= securit(str_replace('|', '', $_POST['v'.$i])) . '|';
			}
		}
		...
		header('location:' . HOST . DIR . '/poll/admin_poll.php');
		exit;
	}
	else
	{
		header('location:' . HOST . DIR . '/poll/admin_poll_add.php?error=incomplete#errorh');
		exit;
	}
}
else	
