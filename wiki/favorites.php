...
if ($add_favorite > 0)//Ajout d'un favori
{
	...
	$is_favorite = $Sql->query("SELECT COUNT(*) FROM " . PREFIX . "wiki_favorites WHERE user_id = '" . $User->get_id() . "' AND id_article = '" . $add_favorite . "'", __LINE__, __FILE__);
	if ($is_favorite == 0)
	{
		$Sql->query_inject("INSERT INTO " . PREFIX . "wiki_favorites (id_article, user_id) VALUES ('" . $add_favorite . "', '" . $User->get_id() . "')", __LINE__, __FILE__);
...
elseif ($remove_favorite > 0)
{
	...
	//On regarde que le sujet n'est pas en favoris
	$is_favorite = $Sql->query("SELECT COUNT(*) FROM " . PREFIX . "wiki_favorites WHERE user_id = '" . $User->get_id() . "' AND id_article = '" . $remove_favorite . "'", __LINE__, __FILE__);
	//L'article est effectivement en favoris
	if ($is_favorite > 0)
	{
		$Sql->query_inject("DELETE FROM " . PREFIX . "wiki_favorites WHERE id_article = '" . $remove_favorite . "' AND user_id = '" . $User->get_id() . "'", __LINE__, __FILE__);
		AppContext::get_response()->redirect('/wiki/' . url('wiki.php?title=' . $article_infos['encoded_title'], $article_infos['encoded_title'], '&'));
	}
	else //Erreur: l'article est déjà en favoris
...
else
{
	...
	$num_rows = $Sql->num_rows($result, "SELECT COUNT(*) FROM " . PREFIX . "wiki_articles WHERE user_id = '" . $User->get_id() . "'", __LINE__, __FILE__);
