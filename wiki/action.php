...
if ($id_auth > 0)
{
	if (!$User->check_auth($config_>get_authorizations(), WIKI_RESTRICTION))
	{
...
if ($id_change_status > 0)
{
	...
	if (!((!$general_auth || $User->check_auth($config_>get_authorizations(), WIKI_STATUS)) && ($general_auth || $User->check_auth($article_auth , WIKI_STATUS))))
	{
		$error_controller = PHPBoostErrors::user_not_authorized();
		DispatchManager::redirect($error_controller);
	} 
...
elseif ($move > 0) //Déplacement d'un article
{
	...
	if (!((!$general_auth || $User->check_auth($config_>get_authorizations(), WIKI_MOVE)) && ($general_auth || $User->check_auth($article_auth , WIKI_MOVE))))
	{
		$error_controller = PHPBoostErrors::user_not_authorized();
		DispatchManager::redirect($error_controller);
	} 
...
elseif ($id_to_rename > 0 && !empty($new_title)) //Renommer un article
{
	...
	if (!((!$general_auth || $User->check_auth($config_>get_authorizations(), WIKI_RENAME)) && ($general_auth || $User->check_auth($article_auth , WIKI_RENAME))))
	{
		$error_controller = PHPBoostErrors::user_not_authorized();
		DispatchManager::redirect($error_controller);
	} 
...
elseif ($del_redirection > 0)//Supprimer une redirection
{
	...
	if ($is_redirection > 0)
	{
		...
		if (!((!$general_auth || $User->check_auth($config_>get_authorizations(), WIKI_REDIRECT)) && ($general_auth || $User->check_auth($article_auth , WIKI_REDIRECT))))
		{
			$error_controller = PHPBoostErrors::user_not_authorized();
			DispatchManager::redirect($error_controller);
		} 
...
elseif (!empty($restore)) //on restaure un ancien article
{
	...
	if (!empty($id_article))
	{
		...
		if (!((!$general_auth || $User->check_auth($config_>get_authorizations(), WIKI_DELETE_ARCHIVE)) && ($general_auth || $User->check_auth($article_auth , WIKI_DELETE_ARCHIVE))))
		{
			$error_controller = PHPBoostErrors::user_not_authorized();
			DispatchManager::redirect($error_controller);
		} 
...
elseif ($del_article > 0) //Suppression d'un article
{
	...
	if (!((!$general_auth || $User->check_auth($config_>get_authorizations(), WIKI_DELETE)) && ($general_auth || $User->check_auth($article_auth , WIKI_DELETE))))
	{
		$error_controller = PHPBoostErrors::user_not_authorized();
		DispatchManager::redirect($error_controller);
	} 
