...
if( $id_auth > 0 )
{
	...
	else
	{
		...
		$array_auth_all = Authorizations::auth_array(WIKI_RESTORE_ARCHIVE, WIKI_DELETE_ARCHIVE, WIKI_EDIT, WIKI_DELETE, WIKI_RENAME, WIKI_REDIRECT, WIKI_MOVE, WIKI_STATUS, WIKI_COM);
...
elseif( !empty($restore) ) //on restaure un ancien article
{
	...
	if( !empty($id_article) )
	{
		...
		if( !((!$general_auth || $Member->check_auth($_WIKI_CONFIG['auth'], WIKI_DELETE_ARCHIVE)) && ($general_auth || $Member->check_auth($article_auth , WIKI_DELETE_ARCHIVE))) )
			$Errorh->Error_handler('e_auth', E_USER_REDIRECT); 
