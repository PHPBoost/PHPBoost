...
if ((!empty($encoded_title) || !empty($id_contents)) && $num_rows > 0)
{
	...
	if ($article_infos['is_cat'] == 1 && $id_contents == 0) //Catégorie non archivée
	{
		...
		$Template->assign_block_vars('cat', array(
			'RSS' => $num_articles > 0 ? '<a href="' . SyndicationUrlBuilder::rss('wiki', $article_infos['id_cat'])  . '"><img src="' . TPL_PATH_TO_ROOT . '/templates/' . get_utheme() . '/images/rss.png" alt="RSS" /></a>' : ''
		));
...
else
{
	if ($_WIKI_CONFIG['last_articles'] > 1)
    {
		...
		$Template->assign_block_vars('last_articles', array(
			...
			'RSS' => $articles_number > 0 ? '<a href="' . SyndicationUrlBuilder::rss('wiki') . '"><img src="../templates/' . get_utheme() . '/images/rss.png" alt="RSS" /></a>' : ''
		));
