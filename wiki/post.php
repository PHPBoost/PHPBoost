...
$is_cat = request_var(POST, 'is_cat', false) ? 1 : 0;
$is_cat_get = (request_var(GET, 'type', '') == 'cat') ? 1 : 0;
$is_cat = $is_cat > 0 ? $is_cat : $is_cat_get;
$id_edit = request_var(POST, 'id_edit', 0);
$title = request_var(POST, 'title', '');
$encoded_title = request_var(GET, 'title', 0);
$contents = wiki_parse(request_var(POST, 'contents', '', TSTRING_UNSECURE));
$contents_preview = request_var(POST, 'contents', '', TSTRING_UNSECURE);
$id_cat = request_var(GET, 'id_parent', 0);
$new_id_cat = request_var(POST, 'id_cat', 0);
$id_cat = $id_cat > 0 ? $id_cat : $new_id_cat;
$preview = !empty($_POST['preview']) ? true : false;
$id_edit_get = request_var(GET, 'id', 0);
