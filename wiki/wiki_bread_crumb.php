...
switch($bread_crumb_key)
{
	case 'wiki':			
		...
		if( !empty($id_cat)  && is_array($_WIKI_CATS) ) //Catégories infinies
		{
			...
			do
			{
				$Bread_crumb->add($_WIKI_CATS[$id]['name'], transid('wiki.php?title=' . url_encode_rewrite($_WIKI_CATS[$id]['name']), url_encode_rewrite($_WIKI_CATS[$id]['name'])));
				$id = (int)$_WIKI_CATS[$id]['id_parent'];
			}	
			while( $id > 0 );
		}
		$Bread_crumb->add((!empty($_WIKI_CONFIG['wiki_name']) ? $_WIKI_CONFIG['wiki_name'] : $LANG['wiki']), transid('wiki.php'));
		$Bread_crumb->reverse();
		break;
	case 'wiki_history':
		$Bread_crumb->add((!empty($_WIKI_CONFIG['wiki_name']) ? $_WIKI_CONFIG['wiki_name'] : $LANG['wiki']),transid('wiki.php'));
		$Bread_crumb->add($LANG['wiki_history'], transid('history.php'));
			if( !empty($id_article) )
				$Bread_crumb->add($article_infos['title'], transid('wiki.php?title=' . $article_infos['encoded_title'], $article_infos['encoded_title']));
		break;
	...
	case 'wiki_property':
		...
		elseif( $wiki_status > 0 )
			$Bread_crumb->add($LANG['wiki_status_management'], transid('property.php?status=' . $article_infos['id']));
		elseif( $move > 0 )
			$Bread_crumb->add($LANG['wiki_moving_article'], transid('property.php?move=' . $move));
		elseif( $rename > 0 )
			$Bread_crumb->add($LANG['wiki_renaming_article'], transid('property.php?rename=' . $rename));
		elseif( $redirect > 0 )
			$Bread_crumb->add($LANG['wiki_redirections'], transid('property.php?redirect=' . $redirect));
		elseif( $create_redirection > 0 )
			$Bread_crumb->add($LANG['wiki_create_redirection'], transid('property.php?create_redirection=' . $create_redirection));
		elseif( isset($_GET['i']) && $idcom > 0 )
			$Bread_crumb->add($LANG['wiki_article_com'], transid('property.php?com=' . $idcom . '&amp;i=0'));
		elseif( $del_article > 0 )
			$Bread_crumb->add($LANG['wiki_remove_cat'], transid('property.php?del=' . $del_article));
	...
	case 'wiki_favorites':
		$Bread_crumb->add((!empty($_WIKI_CONFIG['wiki_name']) ? $_WIKI_CONFIG['wiki_name'] : $LANG['wiki']), transid('wiki.php'));
		$Bread_crumb->add($LANG['wiki_favorites'], transid('favorites.php'));
		break;
	case 'wiki_explorer':
		$Bread_crumb->add(( !empty($_WIKI_CONFIG['wiki_name']) ? $_WIKI_CONFIG['wiki_name'] : $LANG['wiki']), transid('wiki.php'));
		$Bread_crumb->add($LANG['wiki_explorer'], transid('explorer.php'));
		break;
	case 'wiki_search':
		$Bread_crumb->add((!empty($_WIKI_CONFIG['wiki_name']) ? $_WIKI_CONFIG['wiki_name'] : $LANG['wiki']), transid('wiki.php'));
		$Bread_crumb->add($LANG['wiki_search'], transid('search.php'));
		break;
	default:
		$Bread_crumb->add((!empty($_WIKI_CONFIG['wiki_name']) ? $_WIKI_CONFIG['wiki_name'] : $LANG['wiki']), transid('wiki.php'));
		break;
