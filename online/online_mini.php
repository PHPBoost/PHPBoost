...
function online_mini($position, $block)
{
    if (strpos(SCRIPT, '/online/online.php') === false)
    {
    	...
    	$result = $Sql->query_while("SELECT s.user_id, s.expiry, u.level, u.groups, u.display_name
    	FROM " . DB_TABLE_SESSIONS . " s
    	LEFT JOIN " . DB_TABLE_MEMBER . " u ON u.user_id = s.user_id
    		...
    		if ($i < $CONFIG_ONLINE['online_displayed'])
    		{
    			...
    			if ($row['level'] !== '-1')
    			{
					...
					$tpl->assign_block_vars('online', array(
    					'USER' => '<a href="' . TPL_PATH_TO_ROOT . '/member/member' . url('.php?id=' . $row['user_id'], '-' . $row['user_id'] . '.php') . '" class="' . $array_class[$row['level']] . '"' . (!empty($group_color) ? ' style="color:' . $group_color . '"' : '') . '>' . TextHelper::wordwrap_html($row['display_name'], 19) . '</a><br />'
    				));
    				$i++;
    			}
    		}
