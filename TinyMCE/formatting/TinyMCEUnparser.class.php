 ...
 * @version   	PHPBoost 5.2 - last update: 2023 01 17
...
class TinyMCEUnparser extends ContentFormattingUnparser
{
	...
	private function unparse_bbcode_tags()
	{
		...
		##Callbacks
		//FA Icon
		$this->content = preg_replace_callback('`<i class="fa([blrs])? fa-([a-z0-9-]+)( [a-z0-9- ]+)?"( [a-z0-9- ]+)?"(?: aria-hidden="true" title="([^"]+)?")?></i>`iuU', array($this, 'unparse_fa'), $this->content);
