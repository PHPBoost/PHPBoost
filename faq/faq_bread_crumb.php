...
while ($id_cat_for_bread_crumb > 0)
{
	...
	if (is_array($FAQ_CATS[$id_cat_for_bread_crumb]['auth']))
	{
			...
			$auth_read = $auth_read && AppContext::get_current_user()->check_auth($FAQ_CATS[$id_cat_for_bread_crumb]['auth'], FaqAuthorizationsService::READ_AUTHORIZATIONS);
			$auth_write = AppContext::get_current_user()->check_auth($FAQ_CATS[$id_cat_for_bread_crumb]['auth'], FaqAuthorizationsService::WRITE_AUTHORIZATIONS);
	}
