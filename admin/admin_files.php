...
else
{
	...
	while( $row = $Sql->fetch_assoc($result) )
	{
		...
		$Template->assign_block_vars('folder', array(
			...
			'RENAME_FOLDER' => !$show_member ? '<span id="fhref' . $row['id'] . '"><a href="javascript:display_rename_folder(\'' . $row['id'] . '\', \'' . addslashes($row['name']) . '\', \'' . addslashes($name_cut) . '\');" title="' . $LANG['edit'] . '"><img src="../templates/' . uget_theme() . '/images/' . uget_lang() . '/edit.png" alt="" style="vertical-align:middle;" /></a></span>' : '',
			'DEL_TYPE' => $show_member ? 'eptf' : 'delf',
			'DEL_TYPE_IMG' => $show_member ? '<img src="../templates/' . uget_theme() . '/images/upload/trash_mini.png" alt="" class="valign_middle" />' : '<img src="../templates/' . uget_theme() . '/images/' . uget_lang() . '/delete.png" alt="" class="valign_middle" />',
			'ALERT_DEL' => $show_member ? 'member' : 'folder',
			'MOVE' => !$show_member ? '<a href="javascript:upload_display_block(' . $row['id'] . ');" onmouseover="upload_hide_block(' . $row['id'] . ', 1);" onmouseout="upload_hide_block(' . $row['id'] . ', 0);" class="bbcode_hover" title="' . $LANG['moveto'] . '"><img src="../templates/' . uget_theme() . '/images/upload/move.png" alt="" style="vertical-align:middle;" /></a>' : '',
	...
	if( !$show_member ) //Dossier membres.
	{
		...
		while( $row = $Sql->fetch_assoc($result) )
		{
			...
			$Template->assign_block_vars('files', array(
				...
				'URL' => $link,
				'NAME' => $name_cut,
				'RENAME_FILE' => '<span id="fihref' . $row['id'] . '"><a href="javascript:display_rename_file(\'' . $row['id'] . '\', \'' . addslashes($row['name']) . '\', \'' . addslashes($name_cut) . '\');" title="' . $LANG['edit'] . '"><img src="../templates/' . uget_theme() . '/images/' . uget_lang() . '/edit.png" alt="" style="vertical-align:middle;" /></a></span>',
