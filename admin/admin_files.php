...
else
{
	...
	while ($row = $result->fetch())
	{
		...
		$template->assign_block_vars('folder', array(
			...
			'ID' => $row['id'],
			'NAME' => $name_cut,
			'RENAME_FOLDER' => !$show_member ? '<span id="fhref' . $row['id'] . '"><a href="javascript:display_rename_folder(\'' . $row['id'] . '\', \'' . addslashes($row['name']) . '\', \'' . addslashes($name_cut) . '\');" title="' . LangLoader::get_message('edit', 'common') . '"><i class="fa fa-edit"></i></a></span>' : '',
			'DEL_TYPE' => $show_member ? 'eptf' : 'delf',
			'C_TYPEFOLDER' => !$show_member,
			'MOVE' => !$show_member ? '<a href="javascript:upload_display_block(' . $row['id'] . ');" onmouseover="upload_hide_block(' . $row['id'] . ', 1);" onmouseout="upload_hide_block(' . $row['id'] . ', 0);" title="' . $LANG['moveto'] . '"><i class="fa fa-move"></i></a>' : '',
	...
	if (!$show_member) //Dossier membres.
	{
		...
		while ($row = $result->fetch())
		{
			...
			$template->assign_block_vars('files', array(
				...
				'URL' => $link,
				'NAME' => $name_cut,
				'RENAME_FILE' => '<span id="fihref' . $row['id'] . '"><a href="javascript:display_rename_file(\'' . $row['id'] . '\', \'' . addslashes($row['name']) . '\', \'' . addslashes($name_cut) . '\');" title="' . LangLoader::get_message('edit', 'common') . '"><i class="fa fa-edit"></i></a></span>',
				'FILETYPE' => $get_img_mimetype['filetype'] . $size_img,
