...
else
{
	...
	while ($row = $Sql->fetch_assoc($result))
	{
		...
		$template->assign_block_vars('folder', array(
			...
			'RENAME_FOLDER' => !$show_member ? '<span id="fhref' . $row['id'] . '"><a href="javascript:display_rename_folder(\'' . $row['id'] . '\', \'' . addslashes($row['name']) . '\', \'' . addslashes($name_cut) . '\');" title="' . $LANG['edit'] . '" class="pbt-icon-edit"></a></span>' : '',
