...
elseif (!empty($move_folder) && $to != -1) //Déplacement d'un dossier
{
	...
	if (!in_array($to, $array_child_folder)) //Dossier de destination non sous-dossier du dossier source.
		Uploads::Move_folder($move_folder, $to, AppContext::get_current_user()->get_attribute('user_id'), Uploads::ADMIN_NO_CHECK);
	else
...
elseif (!empty($move_file) && $to != -1) //Déplacement d'un fichier
{
	...
	Uploads::Move_file($move_file, $to, AppContext::get_current_user()->get_attribute('user_id'), Uploads::ADMIN_NO_CHECK);
