...
elseif( !empty($_FILES['upload_file']['name']) && isset($_GET['f']) ) //Ajout d'un fichier.
{
	...
	if( is_writable($dir) ) //Dossier en écriture, upload possible
	{
		...
		else //Insertion dans la bdd
		{
			...
			$sql->query("INSERT INTO ".PREFIX."upload (idcat, name, path, user_id, size, type, timestamp) VALUES ('" . $folder . "', '" . securit($_FILES['upload_file']['name']) . "', '" . securit($upload->filename['upload_file']) . "', '" . $user_id . "', '" . numeric(arrondi($_FILES['upload_file']['size']/1024, 1), 'float') . "', '" . $upload->extension['upload_file'] . "', '" . time() . "')", __LINE__, __FILE__);
		}
	}
	else
...
else
{
	...
	FROM ".PREFIX."upload_cat AS uc
	LEFT JOIN ".PREFIX."member AS m ON m.user_id = uc.user_id
	...
	if( !$show_member ) //Dossier membres.
	{
		...
		while( $row = $sql->sql_fetch_assoc($result) )
		{
			...
			$template->assign_block_vars('files', array(
				...
				'SIZE' => ($row['size'] > 1024) ? arrondi($row['size']/1024, 2) . ' ' . $LANG['unit_megabytes'] : arrondi($row['size'], 0) . ' ' . $LANG['unit_kilobytes'],
	...
	$template->assign_vars(array(
		'TOTAL_SIZE' => ($total_size > 1024) ? arrondi($total_size/1024, 2) . ' ' . $LANG['unit_megabytes'] : arrondi($total_size, 0) . ' ' . $LANG['unit_kilobytes'],
		'TOTAL_FOLDER_SIZE' => ($total_folder_size > 1024) ? arrondi($total_folder_size/1024, 2) . ' ' . $LANG['unit_megabytes'] : arrondi($total_folder_size, 0) . ' ' . $LANG['unit_kilobytes'],
