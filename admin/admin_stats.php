...
elseif( !empty($visit) ) //Visites par jour classées par mois.
{
	...
	{	
		...
		$template->assign_block_vars('month', array(
			...
			'U_YEAR' => '<a href="admin_stats.php?year=' . $year . '#stats">' . $year . '</a>',
			'U_VISITS_YEAR' => '<a href="admin_stats.php?year=' . $year . '#stats">' . $LANG['visits_year'] . ' ' . $year . '</a>'
		));
	...
	foreach($stats->data_stats as $key => $angle_value)
	{
		...
		{ 
			...
			$template->assign_block_vars('robots.list', array(
				...
				'VIEWS' => arrondi(($angle_value * $stats->nbr_entry)/360, 0),
				'PERCENT' => arrondi(($angle_value/3.6), 1),
				'DATE' => date($LANG['date_format'], mktime(0, 0, 0, $array_date[$key][1], $array_date[$key][2], $array_date[$key][0])),
