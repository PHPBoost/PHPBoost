			...
			while(($file = readdir($dh)) !== false)
			{
				if( preg_match('`.\.sql$`', $file) )					
				{
					$template->assign_block_vars('list_files.file', array(
						'FILE_NAME' => $file,
						'WEIGHT' => arrondi(filesize($dir . '/' . $file)/1048576, 1) . ' Mo',
						'FILE_DATE' => date($LANG['date_format'], filemtime($dir . '/' . $file))
					));
					$i++;
				}
			}
...
else
{
	...
	if( $action == 'backup' )
	{
		...
		$file_name = 'backup_' . $sql_base . '_' . str_replace('/', '-', date('y-m-d-H-i-s', time())) . '.sql';
	...
	else
	{
		...
		if( !empty($_GET['error']) )
		{
			if( $_GET['error'] == 'backup_success' && !empty($_GET['file']) )
		...
		{	
			$free = arrondi($table_info['data_free']/1024, 1);
			$data = arrondi(($table_info['data_length'] + $table_info['index_lenght'])/1024, 1);
			$free = ($free > 1024) ? arrondi($free/1024, 1) . ' MB' : $free . ' kB';
			$data = ($data > 1024) ? arrondi($data/1024, 1) . ' MB' : $data . ' kB';
		...
		$nbr_free = arrondi($nbr_free/1024, 1);
		$nbr_data = arrondi($nbr_data/1024, 1);
		$nbr_free = ($nbr_free > 1024) ? arrondi($nbr_free/1024, 1) . ' Mo' : $nbr_free . ' Ko';
		$nbr_data = ($nbr_data > 1024) ? arrondi($nbr_data/1024, 1) . ' Mo' : $nbr_data . ' Ko';
