...
elseif( !empty($_FILES['upload_module']['name']) ) //Upload et décompression de l'archive Zip/Tar
{
	...
	{
		$ckeck_module = $sql->query("SELECT COUNT(*) FROM ".PREFIX."modules WHERE name = '" . securit($_FILES['upload_module']['name']) . "'", __LINE__, __FILE__);
		if( empty($ckeck_module) && !is_dir('../' . $_FILES['upload_module']['name']) )
		{
