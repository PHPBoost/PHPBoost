...
{	
	...
	$Sql->Query_inject("UPDATE ".PREFIX."modules_mini SET " . $clause_class . " location = '" . $location . "', activ = '" . $activ . "', auth = '" . securit(serialize($array_auth), HTML_NO_PROTECT) . "' WHERE id = '" . $id_post . "'", __LINE__, __FILE__);
...
elseif( $action == 'install' && !empty($idmodule) ) //Module non installé => insertion dans la bdd
{
	...
	{	
					...
					if( $idmodule == $i )
					{
						...
						{	
							...
							if( empty($check_menu) )
							{
								...
								$Sql->Query_inject("INSERT INTO ".PREFIX."modules_mini (class, name, contents, location, auth, activ, added, use_tpl) VALUES ('" . $class . "', '" . addslashes($module_name) . "', '" . addslashes($path) . "', '" . addslashes($location) . "', '" . securit(serialize($array_auth), HTML_NO_PROTECT) . "', '" . $activ . "', 0, 0)", __LINE__, __FILE__);
		...
		else //Menu perso dans le dossier /menus.
		{
			...
			{
				$class = $Sql->Query("SELECT MAX(class) FROM ".PREFIX."modules_mini WHERE location = '" .  $location . "'", __LINE__, __FILE__) + 1;
				$Sql->Query_inject("INSERT INTO ".PREFIX."modules_mini (class, name, contents, location, auth, activ, added, use_tpl) VALUES ('" . $class . "', '" . str_replace('.php', '', addslashes($module_name)) . "', '" . addslashes($module_name) . "', '" . $location . "', '" . securit(serialize($array_auth), HTML_NO_PROTECT) . "', '" . $activ . "', 2, 0)", __LINE__, __FILE__);
...
{		
	...
	('" . ($class + 1) . "', '" . $name . "', '" . $contents ."', '" . $location . "', '" . securit(serialize($array_auth), HTML_NO_PROTECT) . "', '" . $activ . "', 1, '" . $use_tpl . "')", __LINE__, __FILE__);
