...
if( !empty($table) && $action == 'data' )
{
	...
	while( $row = $Sql->fetch_assoc($result) )
	{
		...
		foreach($row as $field_name => $field_value)
		{
			...
			{
				$Template->assign_block_vars('line.field', array(
					'FIELD' => '<a href="admin_database_tools.php?table=' . $table . '&amp;field=' . $field_name . '&amp;value=' . $field_value . '&amp;action=update" title="' . $LANG['update'] . '"><img src="../templates/' . uget_theme() . '/images/' . uget_lang() . '/edit.png" alt="" class="valign_middle" alt="" /></a> <a href="admin_database_tools.php?table=' . $table . '&amp;field=' . $field_name . '&amp;value=' . $field_value . '&amp;action=delete" onclick="javascript:return Confirm_del_entry()" title="' . $LANG['delete'] . '"><img src="../templates/' . uget_theme() . '/images/' . uget_lang() . '/delete.png" alt="" class="valign_middle" alt="" /></a>',
					'CLASS' => 'row2',
					'STYLE' => ''
				));
			}
