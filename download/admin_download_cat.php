...
elseif ($cat_to_del > 0)
{
	...
	$Template->put_all(array(
		...
		'U_FORM_TARGET' => HOST . DIR . url('/download/admin_download_cat.php?token=' . AppContext::get_session()->get_token())
	));
...
elseif ($new_cat XOR $id_edit > 0)
{
	...
	if ($id_edit > 0 && array_key_exists($id_edit, $DOWNLOAD_CATS))
	{
		$Template->put_all(array(
			...
			'READ_AUTH' => Authorizations::generate_select(DOWNLOAD_READ_CAT_AUTH_BIT, !empty($DOWNLOAD_CATS[$id_edit]['auth']) ? $DOWNLOAD_CATS[$id_edit]['auth'] : $config->get_authorizations()),
			'WRITE_AUTH' => Authorizations::generate_select(DOWNLOAD_WRITE_CAT_AUTH_BIT, !empty($DOWNLOAD_CATS[$id_edit]['auth']) ? $DOWNLOAD_CATS[$id_edit]['auth'] : $config->get_authorizations()),
			'CONTRIBUTION_AUTH' => Authorizations::generate_select(DOWNLOAD_CONTRIBUTION_CAT_AUTH_BIT, !empty($DOWNLOAD_CATS[$id_edit]['auth']) ? $DOWNLOAD_CATS[$id_edit]['auth'] : $config->get_authorizations())
		));
	}
	else
	{
	...
	if (!empty($error))
	{
		switch ($error)
		{
			case 'e_required_fields_empty' :
				$Template->put('message_helper', MessageHelper::display($DOWNLOAD_LANG['required_fields_empty'], MessageHelper::WARNING));
				break;
			case 'e_unexisting_category' :
				$Template->put('message_helper', MessageHelper::display($DOWNLOAD_LANG['unexisting_category'], MessageHelper::WARNING));
				break;
			case 'e_new_cat_does_not_exist' :
				$Template->put('message_helper', MessageHelper::display($DOWNLOAD_LANG['new_cat_does_not_exist'], MessageHelper::WARNING));
				break;
				case 'e_infinite_loop' :
				$Template->put('message_helper', MessageHelper::display($DOWNLOAD_LANG['infinite_loop'], MessageHelper::WARNING));
				break;
			case 'e_success' :
				$Template->put('message_helper', MessageHelper::display($DOWNLOAD_LANG['successful_operation'], MessageHelper::SUCCESS, 4));
				break;
			case 'e_recount_success' :
				$Template->put('message_helper', MessageHelper::display($DOWNLOAD_LANG['recount_success'], MessageHelper::SUCCESS, 4));
				break;
		}
	}
