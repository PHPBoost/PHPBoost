...
<div class="module_position">
	...
	<div class="module_top">{L_PAGE_TITLE}</div>
	<div class="module_contents">
		...
		{SHORT_DESCRIPTION_PREVIEW} # IF C_CONTRIBUTION #
		<h1>{L_CONTRIBUTION_COUNTERPART}</h1>
		{CONTRIBUTION_COUNTERPART_PREVIEW} # ENDIF # # ENDIF #
		...
		<form action="{U_TARGET}" method="post"
			...
			<fieldset>
				...
				<dl>
					...
					<dd>
						<input type="text" size="50" maxlength="100" id="title"
							name="title" value="{TITLE}" class="text" />
					</dd>
				</dl>
				<dl>
					<dt>
				...
				<dl>
					...
					<dd>
						<input type="text" size="50" id="url" name="url" value="{URL}"
							class="text" /> <a title="{L_FILE_ADD}" href="#"
							onclick="window.open('{PATH_TO_ROOT}/member/upload.php?popup=1&amp;fd=url&amp;parse=true', '', 'height=500,width=720,resizable=yes,scrollbars=yes');return false;"><img
							src="{PATH_TO_ROOT}/templates/{THEME}/images/upload/files_add.png"
							alt="" /> </a>
					</dd>
				</dl>
				<dl>
					...
					<dd>
						<input type="text" size="10" maxlength="10" name="size" id="size"
							value="{SIZE_FORM}" class="text" /> {L_UNIT_SIZE}
					</dd>
				</dl>
				<dl>
					<dt>
					...
					<dd>
						<input type="text" size="10" maxlength="10" name="count"
							id="count" value="{COUNT}" class="text" />
					</dd>
				</dl>
				<dl>
					<dt>
						<label for="download_method">{L_DOWNLOAD_METHOD}</label> <br /> <span>{L_DOWNLOAD_METHOD_EXPLAIN}</span>
					</dt>
					<dd>
				...
				<br /> <label for="contents">* {L_CONTENTS}</label> {KERNEL_EDITOR}
				<textarea rows="20" cols="90" id="contents" name="contents">{DESCRIPTION}</textarea>
				<br /> <label for="short_contents">{L_SHORT_CONTENTS}</label>
				{KERNEL_EDITOR_SHORT}
				<textarea rows="20" cols="90" id="short_contents"
					name="short_contents">{SHORT_DESCRIPTION}</textarea>
				<br />
				<dl>
					<dt>
					...
					<dd>
						<input type="text" size="50" name="image" id="image" class="text"
							value="{FILE_IMAGE}" />
					</dd>
				</dl>
				...
				<dl>
					...
					<dd>
						<input type="checkbox" id="ignore_release_date"
							name="ignore_release_date" onclick="show_hide_release_date();" {IGNORE_RELEASE_DATE_CHECKED} />
					</dd>
				</dl>
				<dl id="release_date_form"
					style="display: { STYLE_FIELD_RELEASE_DATE"
					class="overflow_visible">
					<dt>
						<label for="calendar_2">* {L_RELEASE_DATE}</label>
					</dt>
					<dd>{DATE_CALENDAR_RELEASE}</dd>
				</dl>
				<dl class="overflow_visible">
					<dt>
					...
					<dd>
						...
						{L_FROM_DATE}&nbsp; {BEGINING_CALENDAR} &nbsp;{L_TO_DATE}&nbsp;
						{END_CALENDAR} <br /> <label> <input type="radio" value="1"
							name="visibility" {VISIBLE_ENABLED} id="release_date" /> {L_NOW}
						</label> <br /> <label> <input type="radio" value="0"
							name="visibility" {VISIBLE_HIDDEN} /> {L_HIDDEN} </label>
					</dd>
				</dl>
			...
			<fieldset>
				...
				<p>
					<label>{L_CONTRIBUTION_COUNTERPART}</label>
				</p>
			...
			<fieldset class="fieldset_submit">
				...
				&nbsp;&nbsp; <input type="submit" name="preview" value="{L_PREVIEW}"
					...
					value="{L_RESET}" class="reset" /> <input type="hidden"
