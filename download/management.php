...
if ($edit_file_id > 0)
{
	...
	elseif ($preview)
	{
		...
		$Template->put_all(array(
			...
			'CREATION_DATE' => $file_creation_date->format(Date::FORMAT_DAY_MONTH_YEAR) ,
			'RELEASE_DATE' => $file_release_date->get_timestamp() > 0 ? $file_release_date->format(Date::FORMAT_DAY_MONTH_YEAR) : $DOWNLOAD_LANG['unknown_date'],
			'SIZE' => $size_tpl,
			'COUNT' => $file_hits,
		...
		$Template->put_all(array(
			...
			'URL' => $file_url,
			'SIZE_FORM' => $file_size,
			'DATE' => $file_creation_date->format(Date::FORMAT_DAY_MONTH_YEAR, TIMEZONE_AUTO),
	...
	if ($submit)
	{
		...
		if (!empty($file_title) && ($download_categories->check_auth($file_cat_id) || $download_categories->check_contribution_auth($file_cat_id)) && !empty($file_url) && !empty($file_contents))
		{
			...
			if (!$auth_write)
			{
				...
				$download_contribution->set_poster_id(AppContext::get_current_user()->get_id());
				//The module
