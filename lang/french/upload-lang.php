<?php
/**
 * @copyright   &copy; 2005-2020 PHPBoost
 * @license     https://www.gnu.org/licenses/gpl-3.0.html GNU/GPL-3.0
 * @author      Mipel <mipel@phpboost.com>
 * @version     PHPBoost 6.0 - last update: 2021 04 08
 * @since       PHPBoost 6.0 - 2019 11 27
 * @contributor Sebastien LARTIGUE <babsolune@phpboost.com>
*/

####################################################
#                     French                       #
####################################################

$lang['upload.files.management'] = 'Gestion des fichiers';
$lang['upload.files.config']     = 'Configuration des fichiers';
$lang['upload.file.add']         = 'Ajouter un fichier';
$lang['upload.files.add']        = 'Ajout de fichiers';
$lang['upload.total.datas']      = 'Total des données';
$lang['upload.folders']          = 'Répertoires';
$lang['upload.folder.up']        = 'Répertoire parent';
$lang['upload.folder.new']       = 'Nouveau dossier';
$lang['upload.folder.content']   = 'Contenu du répertoire';
$lang['upload.root']             = 'Racine';
$lang['upload.files']            = 'Fichiers';
$lang['upload.folder.size']      = 'Taille du dossier';
$lang['upload.file.type']        = 'Fichier %s';
$lang['upload.image.type']       = 'Image %s';
$lang['upload.audio.type']       = 'Fichier audio %s';
$lang['upload.zip.type']         = 'Archive %s';
$lang['upload.adobe.pdf']        = 'Adobe Document';
$lang['upload.document.type']    = 'Document %s';
$lang['upload.popup.insert']     = 'Insérer le code dans le formulaire';

$lang['upload.shared.checkbox'] = 'Cocher cette case pour que les fichiers soient enregistrés comme fichiers partagés';
$lang['upload.shared.files'] = 'Fichiers Partagés';
$lang['upload.personal.files'] = 'Fichiers Personnels';
$lang['upload.change.to.personal'] = 'Changer en Personnel';
$lang['upload.change.to.shared'] = 'Changer en Partagé';
$lang['upload.posted.by'] = 'Déposé par : ';

$lang['upload.drag.and.drop.file'] = 'Cliquer ici ou déposer un fichier';
$lang['upload.drag.and.drop.files'] = 'Cliquer ici ou déposer des fichiers';
$lang['upload.click.and.add.file'] = 'Cliquer ici pour ajouter un fichier';
$lang['upload.click.and.add.files'] = 'Cliquer ici pour ajouter des fichiers';
$lang['upload.upload.helper'] = 'Options autorisées pour le téléversement';
$lang['upload.clear.list'] = 'Vider la liste';
$lang['upload.max.file.size'] = 'Taille maximum par fichier';
$lang['upload.max.files.size'] = 'Espace alloué';
$lang['upload.allowed.extensions'] = 'Extensions de fichier autorisées';
$lang['upload.warning.upload.disabled'] = '<strong>Le téléversement a été désactivé pour une des raisons suivantes : <br /></strong>';
$lang['upload.warning.upload.extension'] = ' : l\'extension de ce fichier n\'est pas autorisée.<br />';
$lang['upload.warning.upload.file.size'] = ' : le poids de ce fichiers est trop important.<br />';
$lang['upload.warning.upload.file.dim'] = 'La hauteur ou la largeur d\'une image excède les dimensions autorisées.<br />';
$lang['upload.warning.upload.files.number'] = '<br />- l\'ensemble des fichiers dépasse l\'espace alloué.<br />';
?>
