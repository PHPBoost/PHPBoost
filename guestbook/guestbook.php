...
if( !$groups->check_auth($SECURE_MODULE['guestbook'], ACCESS_MODULE) )
{
...
{
	$guestbook_contents = !empty($_POST['guestbook_contents']) ? $_POST['guestbook_contents'] : '';
...
else //Affichage.
{
	...
	FROM ".PREFIX."guestbook as g
	LEFT JOIN ".PREFIX."member AS m ON m.user_id = g.user_id
	LEFT JOIN ".PREFIX."sessions AS s ON s.user_id = g.user_id AND s.session_time > '" . (time() - $CONFIG['site_session_invit']) . "'
	GROUP BY g.id
