...
class GuestbookInterface extends ModuleInterface
{
	...
	function get_cache()
	{
		...
		$CONFIG_GUESTBOOK = unserialize($this->db_connection->query("SELECT value FROM " . DB_TABLE_CONFIGS . " WHERE name = 'guestbook'", __LINE__, __FILE__));
		...
		$result = $this->db_connection->query_while("SELECT g.id, g.login, g.user_id, g.timestamp, m.login as mlogin, g.contents
		FROM " . PREFIX . "guestbook g
		LEFT JOIN " . DB_TABLE_MEMBER . " m ON m.user_id = g.user_id
		ORDER BY g.timestamp DESC
		" . $this->db_connection->limit(0, 10), __LINE__, __FILE__);
