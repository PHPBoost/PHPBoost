    ...
    if (strpos(SCRIPT, '/guestbook/guestbook.php') === false)
    {
		...
		else
		{
	    	...
	    	if ($guestbook_rand['user_id'] != -1)
	    		$guestbook_login = '<a class="small_link" href="' . MemberUrlBuilder::profile($guestbook_rand['user_id'])->absolute() . '" title="' . $guestbook_rand['login'] . '"><span style="font-weight:bold;">' . TextHelper::wordwrap_html($guestbook_rand['login'], 13) . '</span></a>';
	    	else
