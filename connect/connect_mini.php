...
if ($User->check_level(MEMBER_LEVEL)) //Connecté.
{
	...
	$Template->assign_vars(array(
		...
		'U_MEMBER_PM' => PATH_TO_ROOT . '/member/pm' . url('.php?pm=' . $User->get_attribute('user_id'), '-' . $User->get_attribute('user_id') . '.php'),
		'U_MEMBER_ID' => url('.php?id=' . $User->get_attribute('user_id') . '&amp;view=1', '-' . $User->get_attribute('user_id') . '.php?view=1'),
