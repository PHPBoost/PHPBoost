...
if( $User->check_level(MEMBER_LEVEL) ) //Connecté.
{
	...
	$Template->Assign_vars(array(
		...
		'U_MEMBER_PM' => PATH_TO_ROOT . '/member/pm' . transid('.php?pm=' . $User->get_attribute('user_id'), '-' . $User->get_attribute('user_id') . '.php'),
		'U_MEMBER_ID' => transid('.php?id=' . $User->get_attribute('user_id') . '&amp;view=1', '-' . $User->get_attribute('user_id') . '.php?view=1'),
