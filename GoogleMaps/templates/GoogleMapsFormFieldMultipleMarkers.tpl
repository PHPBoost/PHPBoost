...
GoogleMapsFormFieldMultipleMarkers.prototype = {
	add : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<input/> ', {type : 'text', id : id, name : id, required : "required", placeholder : '{@form.marker.address}', class: 'multiple-markers# IF C_CLASS # ${escape(CLASS)}# ENDIF #'}).appendTo('#field-' + id);
			jQuery('#field-' + id).append(' ');
			
			jQuery('<input/> ', {type : 'text', id : 'name-' + id, name : 'name-' + id, placeholder : '{@form.marker.name}', class: 'multiple-markers marker-name# IF C_CLASS # ${escape(CLASS)}# ENDIF #'}).appendTo('#field-' + id);
			jQuery('#field-' + id).append(' ');
...
# START fieldelements #
	<div id="marker-${escape(HTML_ID)}-{fieldelements.ID}">
		<div id="field-${escape(HTML_ID)}-{fieldelements.ID}">
			...
			<input type="text" name="name-${escape(HTML_ID)}-{fieldelements.ID}" id="name-${escape(HTML_ID)}-{fieldelements.ID}" value="{fieldelements.MARKER_NAME}" placeholder="{@form.marker.name}" class="multiple-markers marker-name# IF C_READONLY # low-opacity# ENDIF ## IF C_CLASS # ${escape(CLASS)}# ENDIF #" # IF C_DISABLED # disabled="disabled" # ENDIF # />
