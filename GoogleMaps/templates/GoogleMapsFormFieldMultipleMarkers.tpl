...
GoogleMapsFormFieldMultipleMarkers.prototype = {
	add : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<a/> ', {href : 'javascript:GoogleMapsFormFieldMultipleMarkers.delete('+ this.integer +');', title : "${LangLoader::get_message('delete', 'common')}"}).html('<i class="far fa-delete"></i>').appendTo('#field-' + id);
...
# START fieldelements #
	<div id="marker-${escape(HTML_ID)}-{fieldelements.ID}" class="marker-container">
		<div id="field-${escape(HTML_ID)}-{fieldelements.ID} map-input-container">
			...
			<a href="javascript:GoogleMapsFormFieldMultipleMarkers.delete({fieldelements.ID});" title="${LangLoader::get_message('delete', 'common')}" data-confirmation="delete-element"><i class="far fa-delete"></i></a>
		</div>
