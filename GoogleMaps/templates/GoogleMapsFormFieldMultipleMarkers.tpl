...
GoogleMapsFormFieldMultipleMarkers.prototype = {
	add : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<a/> ', {href : 'javascript:GoogleMapsFormFieldMultipleMarkers.delete('+ this.integer +');', 'aria-label' : '{@form.del.marker}'}).html('<i class="fa fa-trash-alt" aria-hidden="true"></i>').appendTo('#field-' + id);
...
# START fieldelements #
	<div id="marker-${escape(HTML_ID)}-{fieldelements.ID}" class="marker-container">
		<div class="grouped-inputs map-input-container" id="field-${escape(HTML_ID)}-{fieldelements.ID} map-input-container">
			...
			<a href="javascript:GoogleMapsFormFieldMultipleMarkers.delete({fieldelements.ID});" aria-label="{@form.del.marker}" data-confirmation="delete-element"><i class="fa fa-trash-alt" aria-hidden="true"></i></a>
		</div>
