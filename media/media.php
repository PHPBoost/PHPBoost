	...
	if ($MEDIA_CATS[$id_cat]['num_media'] > 0)
	{
		...
		while ($row = $Sql->fetch_assoc($result))
		{
			...
			$Template->assign_block_vars('file', array(
				...
				'POSTER' => !empty($row['login']) ? sprintf($MEDIA_LANG['media_added_by'], $row['login'], MemberUrlBuilder::profile($row['iduser'])->absolute(), $level[$row['level']]) : $LANG['guest'],
...
elseif ($id_media > 0)
{
	...
	$Template->put_all(array(
		...
		'L_VIEWED' => $LANG['view'],
		'L_BY' => $LANG['by'],
		'BY' => !empty($media['login']) ? sprintf($MEDIA_LANG['media_added'], $media['login'], MemberUrlBuilder::profile($media['iduser'])->absolute(), $level[$media['level']]) : $LANG['guest'],
