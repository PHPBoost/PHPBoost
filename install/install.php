...
elseif( $step == 4 )
{
	if( !empty($_POST['submit']) )
	{
		...
		if( empty($error) )
		{
			...
			$file = @fopen($file_path, 'w+'); //On ouvre le fichier, si il n'existe pas on le cr�e.
            if ( $dbms != 'SQLite' )
            {
                @fputs($file, '<?php
if( !defined(\'DBSECURE\') )
{
    ...
    $sql_base = "' . $database . '"; //Nom de la base de donn�es.
    $table_prefix = "' . $tableprefix . '"; //Pr�fixe des tables
    $dbtype = "' . $dbms .'"; //Syst�me de gestion de base de donn�es
...
if( !defined(\'DBSECURE\') )
{
    ...
    $sql_login = "' . $login . '"; //Login
    $sql_pass = "' . $password . '"; //Mot de passe
    $sql_base = "../kernel/framework/db/' . $database . '.sqlite"; //Nom de la base de donn�es (url du fichier SQLite).
    $table_prefix = "' . $tableprefix . '"; //Pr�fixe des tables
    $dbtype = "' . $dbms .'"; //Syst�me de gestion de base de donn�es
    define(\'DBSECURE\', true);
    define(\'PHPBOOST_INSTALLED\', true);
...
elseif( $step == 5 )
{
	...
	if( !empty($_POST['submit']) )
	{
		...
		//On ins�re la langue dans la bdd.
		...
		//On ins�re le th�me dans la bdd.
	...
	if( is_dir($rep) ) //Si le dossier existe
	{
		...
		{	
			//Si c'est un r�pertoire un regarde si c'est effectivement un dossier de langues
			if( !preg_match('`\.`', $file) )
			{
...
elseif( $step == 6 )
{
	...
	if( !empty($_POST['submit']) )
	{
		...
		if( empty($error) )
		{
			...
			$unlock_admin = substr(strhash(uniqid(mt_rand(), true)), 0, 12); //G�n�ration de la cl�e d'activation, en cas de verrouillage de l'administration.;
...
elseif( $step == 7 )
{
	//Liste des modules support�s par l'installateur
	...
	//Validation : installation des modules demand�s
	if( !empty($_POST['submit']) )
	{
		...
		//L'utilisateur a choisi une pr�selection
		if( in_array($preselection, array('no_one', 'all', 'community', 'publication')) )
		{
			//Configuration des pr�selections
			...
			foreach( $preselections_configs[$preselection][1] as $module_name )
			{
				if( !in_array($module_name, $unexisting_modules) )
				{
					...
					//Si le dossier de base de donn�es de la langue n'existe pas on prend le suivant exisant.
					$dir_db_module = $lang;
					$dir = '../' . $module_name . '/db';
		...
		//Sinon combinaison personnalis�e
		else
		{
			foreach($supported_modules as $module_name )
			{
			...
			//D�sactivation de l'espace membre dans le cas o� c'est demand�
			if( !$activ_member )
			{
				$Cache->Load_file('member');
			...
			//Traitement de la page de d�marrage
		...
		//On r�assigne le class des modules minis
		$i = 0;
		...
		//On ins�re les liens dans le module de liens, s'il est install�.
		if( $link_installed )
		{
			$i = 6;
