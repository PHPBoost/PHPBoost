...
class InstallationServices
{
	...
	private function load_distribution_configuration()
	{
		...
		if (!empty($modules) && !in_array($this->distribution_config['module_home_page'], $modules))
			$this->distribution_config['module_home_page'] = $modules[key($modules)]->get_id();
		$themes = $this->get_themes_not_installed();
		if (!empty($themes) && !in_array($this->distribution_config['default_theme'], $themes))
			$this->distribution_config['default_theme'] = $themes[key($themes)]->get_id();
		$langs = $this->get_langs_not_installed();
		if (!empty($langs) && !in_array($this->distribution_config['default_lang'], $langs))
