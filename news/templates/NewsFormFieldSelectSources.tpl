...
NewsFormFieldSelectSources.prototype = {
	add_field : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<a/> ', {href : 'javascript:NewsFormFieldSelectSources.delete_field('+ this.integer +');'}).html('<i class="far fa-delete"></i>').appendTo('#' + id);
...
# START fieldelements #
	<div id="${escape(ID)}_{fieldelements.ID}">
		...
		<a href="javascript:NewsFormFieldSelectSources.delete_field({fieldelements.ID});" data-confirmation="delete-element"><i class="far fa-delete"></i></a>
	</div>
# END fieldelements #
</div>
<a href="javascript:NewsFormFieldSelectSources.add_field();" id="add-${escape(ID)}" class="field-source-more-value" title="${LangLoader::get_message('add', 'common')}"><i class="fas fa-plus"></i></a>
