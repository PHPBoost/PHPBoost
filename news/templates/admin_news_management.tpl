			...
			-->
				...
				<form action="admin_news.php?token={TOKEN}" name="form" method="post" style="margin:auto;" onsubmit="return check_form();" class="fieldset_content">
					<fieldset>
						...
						{KERNEL_EDITOR}
						<label><textarea type="text" rows="20" cols="86" id="contents" name="contents">{news.CONTENTS}</textarea></label>
						<br /><br />
						<label for="extend_contents">{L_EXTENDED_NEWS}</label>
						{KERNEL_EDITOR_EXTEND}
						<label><textarea type="text" rows="20" cols="86" id="extend_contents" name="extend_contents">{news.EXTEND_CONTENTS}</textarea></label>
						<br />
