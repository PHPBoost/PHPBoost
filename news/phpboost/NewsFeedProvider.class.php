...
class NewsFeedProvider implements FeedProvider
{
	...
	public function get_feed_data_struct($idcat = 0, $name = '')
	{
		...
		if (CategoriesService::get_categories_manager($module_id)->get_categories_cache()->category_exists($idcat))
		{
			...
			$now = new Date();
			$results = $querier->select('SELECT news.id, news.id_category, news.title, news.rewrited_title, news.content, news.summary, news.creation_date, news.thumbnail_url, cat.rewrited_name AS rewrited_name_cat
