...
class Newsletter_sender extends Sql
{
	function Newsletter_sender(&$req)
    {
        parent::Sql();    
	...
	{
		global $_NEWSLETTER_CONFIG, $LANG;
		...
		{
			$result = parent::query_while("SELECT id, mail 
			...
			$nbr = parent::count_table('newsletter', __LINE__, __FILE__);
...
<head><title>' . $mail_object . '</title></head><body>';
		...
		if( $email_test == '' ) // envoi définitif
		{
			...
			while( $row = parent::sql_fetch_assoc($result) )
			{
				$mail_contents_end = '<br /><br /><a href="' . HOST . DIR . '/newsletter/newsletter.php?id=' . $row['id'] . '">' . $LANG['newsletter_unscubscribe_text'] . '</a></body></html>';
				if( !@mail($row['mail'], $mail_object, $mail_contents . $mail_contents_end, $headers) )
					$error_mailing_list[] = $row['mail'];
			}
			$nbr = parent::count_table('newsletter', __LINE__, __FILE__);
			//On enregistre dans les archives la newsletter envoyée
			parent::query_inject("INSERT INTO ".PREFIX."newsletter_arch (title,message,timestamp,type,nbr) VALUES('" . securit($mail_object) . "', '" . addslashes($message) . "', '" . time() . "', 'bbcode', '" . $nbr . "')", __LINE__, __FILE__);
			return $error_mailing_list;
		}
		else
		{
	...
	function send_text($mail_object, $message, $email_test = '')
	{
		...
		if( $email_test == '' ) // envoi définitif
		{
			...
			$nbr = parent::count_table('newsletter', __LINE__, __FILE__);
			//On enregistre dans les archives la newsletter envoyée
			parent::query_inject("INSERT INTO ".PREFIX."newsletter_arch (title,message,timestamp,type,nbr) VALUES('" . $mail_object . "', '" . addslashes($message) . "', '" . time() . "', 'text', '" . $nbr . "')", __LINE__, __FILE__);
			return $error_mailing_list;
		}
		else
		{
