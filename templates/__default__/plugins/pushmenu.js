/**
 * HC Off-canvas Nav - Version: 4.1.4
 * @copyright   &copy; 2005-2022 PHPBoost - 2014 Some Web Media
  ...
  const deprecated = (() => {
    ...
    return (what, instead, type) => {
      console.warn(
        ...
        + "%c '" + instead + "'"
        + '%c option instead. See details about plugin usage at https://github.com/somewebmedia/pushmenu.',
        'color: #fa253b',
        'color: default',
        'color: #5595c6',
        'color: default',
  ...
  $.fn.extend({
    pushmenu: function(options = {}) {
      ...
      const Plugin = function() {
        ...
        const Styles = printStyle(`pushmenu-id-${navCount}-style`);
        ...
        $originalNav.addClass(`push-nav-original ${navUniqId}`);
        ...
        if (!Settings.customToggle) {
          ...
          $toggle = $(Settings.customToggle)
            .addClass(`push-nav-trigger ${navUniqId}`)
            .on('click', toggleNav);
        }
        ...
        const calcNav = () => {
          ...
          Styles.add(`.pushmenu.${navUniqId}.nav-position-left .nav-container`, `transform: translate3d(-${_containerWidth}px, 0, 0)`);
          Styles.add(`.pushmenu.${navUniqId}.nav-position-right .nav-container`, `transform: translate3d(${_containerWidth}px, 0, 0)`);
          Styles.add(`.pushmenu.${navUniqId}.nav-position-top .nav-container`, `transform: translate3d(0, -${_containerHeight}px, 0)`);
          Styles.add(`.pushmenu.${navUniqId}.nav-position-bottom .nav-container`, `transform: translate3d(0, ${_containerHeight}px, 0)`);
        ...
        const initNav = (reinit) => {
          ...
          // container transform
          Styles.add(`.pushmenu.${navUniqId}.nav-position-left .nav-container`, `transform: translate3d(-${width === 'auto' ? '100%' : width}, 0, 0);`);
          Styles.add(`.pushmenu.${navUniqId}.nav-position-right .nav-container`, `transform: translate3d(${width === 'auto' ? '100%' : width}, 0, 0);`);
          Styles.add(`.pushmenu.${navUniqId}.nav-position-top .nav-container`, `transform: translate3d(0, -${height === 'auto' ? '100%' : height}, 0);`);
          Styles.add(`.pushmenu.${navUniqId}.nav-position-bottom .nav-container`, `transform: translate3d(0, ${height === 'auto' ? '100%' : height}, 0);`);

          // wrappers
          Styles.add(`.pushmenu.${navUniqId}.nav-levels-overlap.nav-position-left li.level-open > .nav-wrapper`, `transform: translate3d(-${Settings.levelSpacing}px,0,0)`, mediaquery);
          Styles.add(`.pushmenu.${navUniqId}.nav-levels-overlap.nav-position-right li.level-open > .nav-wrapper`, `transform: translate3d(${Settings.levelSpacing}px,0,0)`, mediaquery);
          Styles.add(`.pushmenu.${navUniqId}.nav-levels-overlap.nav-position-top li.level-open > .nav-wrapper`, `transform: translate3d(0,-${Settings.levelSpacing}px,0)`, mediaquery);
          Styles.add(`.pushmenu.${navUniqId}.nav-levels-overlap.nav-position-bottom li.level-open > .nav-wrapper`, `transform: translate3d(0,${Settings.levelSpacing}px,0)`, mediaquery);
                  ...
                  // if we can open levels
                    ...
                    // item has real link
                      ...
                      attachToLink(
                        ...
                        $(`<a name="${nav_title}" class="nav-next" aria-label="${nav_title}" role="menuitem" tabindex="0"><span>`)
                          .on('click', stopPropagation)
                          .insertAfter($a)
                      );
                    }
        ...
        function openNav() {
          ...
          if (Settings.disableBody) {
            ...
            if (hasScrollBar()) {
              $html.addClass('push-nav-yscroll');
            }
        ...
        function closeNav() {
          ...
          if (Settings.disableBody) {
            ...
            $html.removeClass('push-nav-yscroll');
