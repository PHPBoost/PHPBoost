...
FormFieldSelectSources.prototype = {
	add_field : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<a/> ', {href : 'javascript:FormFieldSelectSources.delete_field('+ this.integer +');', class : 'grouped-element bgc-full error', 'aria-label' : '{@form.del.source}'}).html('<i class="far fa-trash-alt" aria-hidden="true"></i>').appendTo('#' + id);
...
<div id="input_fields_${escape(ID)}">
	# START fieldelements #
		<div class="sources-link grouped-inputs" id="${escape(ID)}_{fieldelements.ID}">
			...
			<a class="grouped-element bgc-full error" href="javascript:FormFieldSelectSources.delete_field({fieldelements.ID});" data-confirmation="delete-element" aria-label="{@form.del.source}"><i class="far fa-trash-alt" aria-hidden="true"></i></a>
		</div>
	# END fieldelements #
</div>
