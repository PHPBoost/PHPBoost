...
<div
	...
	class="form-element form-element-checkbox# IF C_REQUIRED_AND_HAS_VALUE # constraint-status-right# ENDIF ## IF C_HAS_CSS_CLASS # {CSS_CLASS}# ENDIF #"
	...
	<div id="onblurContainerResponse${escape(HTML_ID)}" class="form-field# IF C_HAS_FORM_FIELD_CLASS # {FORM_FIELD_CLASS}# ENDIF # picture-status-constraint# IF C_REQUIRED # field-required # ENDIF #">
		<label class="checkbox" for="${escape(HTML_ID)}">
			<input
				...
				# IF C_CHECKED # checked="checked" # ENDIF #
				# IF C_READONLY # readonly="readonly" # ENDIF #/>
			<span>&nbsp;<span class="sr-only">${LangLoader::get_message('common.select', 'common-lang')}</span></span>
		</label>
