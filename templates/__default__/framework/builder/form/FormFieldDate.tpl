...
<div
	...
	class="form-element form-element-date# IF C_HOUR # form-element-date-time# ENDIF ## IF C_REQUIRED_AND_HAS_VALUE # constraint-status-right# ENDIF ## IF C_HAS_CSS_CLASS # {CSS_CLASS}# ENDIF #"
		...
		# IF C_HOUR #
			<label for="${escape(HTML_ID)}_hours" class="label-time grouped-element" data-time="{L_H}">
				<input
					...
					min="0"
					max="23"
					id="${escape(HTML_ID)}_hours"
					class="input-hours"
					name="${escape(HTML_ID)}_hours"
					value="{HOURS}"
					# IF C_DISABLED # disabled="disabled"# ENDIF #
					# IF C_READONLY # readonly="readonly"# ENDIF #/>
				<span class="sr-only">${LangLoader::get_message('date.hours', 'date-lang')}</span>
			</label>
			<label for="${escape(HTML_ID)}_minutes" class="label-time grouped-element" data-time="{L_MN}">
				<input
					...
					min="0"
					max="59"
					# IF C_FIVE_MINUTES_STEP # step="5"# ENDIF #
					id="${escape(HTML_ID)}_minutes"
					class="input-minutes"
					name="${escape(HTML_ID)}_minutes"
					value="{MINUTES}"
					# IF C_DISABLED # disabled="disabled"# ENDIF #
					# IF C_READONLY # readonly="readonly"# ENDIF #/>
				<span class="sr-only">${LangLoader::get_message('date.minutes', 'date-lang')}</span>
			</label>
		# ENDIF #
	</div>
</div>
