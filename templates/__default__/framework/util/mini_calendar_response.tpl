<span class="error big hide-modal close-modal align-right" aria-label="{@common.close}"><i class="far fa-circle-xmark" aria-hidden="true"></i></span>
<div class="cell-body date-select-container">
			...
			<tr>
				# START day #
                    <td class="{day.CLASS} square-cell">
                        <span class="close-modal calendar-day" onclick="insert_date(${escapejs(FIELD)}, ${escapejs(INPUT_FIELD)}, ${escapejs(day.DATE)}, ${escapejs(CALENDAR_NUMBER)});return false;">{day.DAY}</span>
                    </td>
				# IF day.CHANGE_LINE #
			</tr>
			<tr>
...
<script>
    document.querySelectorAll('.close-modal').forEach(element => {
        element.addEventListener('click', () => {
            element.closest('.modal').classList.remove('active-modal');
        });
    });
</script>