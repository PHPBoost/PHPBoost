...
<div class="calendar-container modal-container cell-modal cell-tile grouped-element">
	<a class="bgc-sub modal-button --calendar{CALENDAR_NUMBER}" id="{CALENDAR_ID}_link" href="#" onclick="xmlhttprequest_calendar('{CALENDAR_ID}_date', ${escapejs(CALENDAR_ID)}, ${escapejs(YEAR)}, ${escapejs(MONTH)}, ${escapejs(DAY)}, ${escapejs(CALENDAR_NUMBER)}, 1);return false;" aria-label="{@form.date.selector}"><i class="fa fa-calendar-alt" aria-hidden="true"></i></a>
	<div id="calendar{CALENDAR_NUMBER}" class="modal modal-calendar">
		<div class="modal-overlay close-modal"></div>
		<div id="{CALENDAR_ID}_date" class="modal-content cell calendar-block"></div>
	</div>
</div>
