	...
	<div class="horizontal-fieldset">
		...
		<div class="connect-content">
			<form action="{U_CONNECT}" method="post" onsubmit="return check_connect();">
				...
				<input checked="checked" id="autoconnect" type="checkbox" name="autoconnect" title="{@autoconnect}"><span class="hidden-large-screens">{@autoconnect}</span>
				<input type="hidden" name="redirect" value="{SITE_REWRITED_SCRIPT}">
				<input type="hidden" name="token" value="{TOKEN}">
				<button type="submit" id="btn-connect" name="authenticate" value="internal" class="submit">{@connection}</button>
			</form>
			# IF C_USER_REGISTER #
				<form action="${relative_url(UserUrlBuilder::registration())}" method="post">
					<button type="submit" id="btn-register" name="register" value="true" class="submit">{@register}</button>
					<input type="hidden" name="token" value="{TOKEN}">
				</form>
...
<div id="connect-menu">
	<div class="horizontal-fieldset">
		...
		<a href="" class="js-menu-button" onclick="open_submenu('connect-menu');return false;" title="{@dashboard}">{@dashboard} <i class="fa fa-bars # IF NUMBER_TOTAL_ALERT # blink alert# ENDIF #"></i></a>
		<ul class="connect-content">
			...
			<li class="connect-disconnect">
				<a href="${relative_url(UserUrlBuilder::disconnect())}" class="small" title="{@disconnect}"><i class="fa fa-sign-out"></i> <span class="hidden-large-screens"><span>{@disconnect}</span></a>
			</li>
		</ul>
