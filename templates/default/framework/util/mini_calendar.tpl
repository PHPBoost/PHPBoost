...
function insert_date(field, input_field, date, calendar_number) 
{
	if (jQuery('\#' + input_field)) {
		if (date) {
			...
			month = array[1];
			day = array[2];
			jQuery('\#' + input_field + '_link').attr('href', "javascript:xmlhttprequest_calendar('" + field + "', '" + input_field + "', '" + year + "', '" + month + "', '" + day + "', '" + calendar_number + "', 1);");
			jQuery('\#' + input_field).attr('onclick', "javascript:xmlhttprequest_calendar('" + field + "', '" + input_field + "', '" + year + "', '" + month + "', '" + day + "', '" + calendar_number + "');return false;");
...
<div class="calendar-container">
	<a id="{CALENDAR_ID}_link" href="javascript:xmlhttprequest_calendar('{CALENDAR_ID}_date', ${escapejs(CALENDAR_ID)}, ${escapejs(YEAR)}, ${escapejs(MONTH)}, ${escapejs(DAY)}, ${escapejs(CALENDAR_NUMBER)}, 1);"><i class="fa fa-calendar"></i></a>
