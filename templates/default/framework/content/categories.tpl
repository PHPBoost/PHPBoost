...
function ajax_move_cat(id, direction)
{
	...
	var xhr_object = xmlhttprequest_init('{CONFIG_XMLHTTPREQUEST_FILE}?id_' + direction + '=' + id + "&token={TOKEN}");
...
function ajax_change_cat_visibility(id, status)
{
	...
	var xhr_object = xmlhttprequest_init('{CONFIG_XMLHTTPREQUEST_FILE}?' + status + '=' + id + "&token={TOKEN}");
