...
<input type="text" size="8" name="${escape(NAME)}" id="${escape(ID)}" value="${escape(VALUE)}" style="background-color:${escape(VALUE)};" class="text" # IF C_DISABLED # disabled="disabled" # ENDIF # # IF C_HIDDEN # style="display:none;" # ENDIF # />
<a href="javascript:${escape(NAME)}bbcode_color('${escape(ID)}');bb_display_block('3', '');" onmouseout="bb_hide_block('3', '', 0);" class="bbcode_hover">
...
<div style="position:relative;z-index:100;display:none;" id="bb_block3">
	<div id="${escape(ID)}_list" class="bbcode_block" onmouseover="bb_hide_block('3', '', 1);" onmouseout="bb_hide_block('3', '', 0);">
	</div>
</div>
