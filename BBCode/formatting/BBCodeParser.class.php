...
class BBCodeParser extends ContentFormattingParser
{
	...
	protected function parse_simple_tags()
	{
		...
		$array_preg = array(
			...
			'url1' => '`\[url\]((?!javascript:)' . Url::get_wellformness_regex() . ')\[/url\]`suU',
			'url2' => '`\[url=((?!javascript:)' . Url::get_wellformness_regex() . ')\]([^\n\r\t\f]+)\[/url\]`suU',
			'url3' => '`(\s+)(' . Url::get_wellformness_regex(RegexHelper::REGEX_MULTIPLICITY_REQUIRED) . ')(\s|<+)`suU',
			'url4' => '`(\s+)(www\.' . Url::get_wellformness_regex(RegexHelper::REGEX_MULTIPLICITY_NOT_USED) . ')(\s|<+)`suU',
		...
		$array_preg_replace = array(
			...
			{
				$this->content = preg_replace_callback('`\[fa(=[a-z0-9-]+)?(,[a-z0-9-]+)?(,[a-z0-9-]+)?\]([a-z0-9-]+)\[/fa\]`iuU', array($this, 'parse_fa'), $this->content);
			}
			...
			{
				$this->_parse_imbricated('[hide]', '`\[hide\](.+)\[/hide\]`suU', '<div class="formatter-container formatter-hide"><span class="formatter-title">' . LangLoader::get_message('hidden', 'common') . ' :</span><div class="formatter-content">$1</div></div>');
				$this->_parse_imbricated('[hide=', '`\[hide=([^\]]+)\](.+)\[/hide\]`suU', '<div class="formatter-container formatter-hide"><span class="formatter-title title-perso">$1 :</span><div class="formatter-content">$2</div></div>');
			}
