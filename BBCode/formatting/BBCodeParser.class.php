...
class BBCodeParser extends ContentFormattingParser
{
	...
	protected function parse_simple_tags()
	{
		...
		$array_preg = array(
			...
			'url3' => '`(\s+)(' . Url::get_wellformness_regex(RegexHelper::REGEX_MULTIPLICITY_REQUIRED) . ')(\s|<+)`isuU',
			'url4' => '`(\s+)(www\.' . Url::get_wellformness_regex(RegexHelper::REGEX_MULTIPLICITY_NOT_USED) . ')(\s|<+)`isuU',
