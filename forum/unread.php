...
if( $session->check_auth($session->data, 0) ) //Affichage des message()s non lu(s) du membre.
{
	...
	LEFT JOIN ".PREFIX."forum_view AS v ON v.idtopic = t.id	AND v.user_id = '" . $session->data['user_id'] . "'
	...
	WHERE s.session_time > '" . (time() - $CONFIG['site_session_invit']) . "' AND s.session_script = '" . DIR . "/forum/unread.php'
