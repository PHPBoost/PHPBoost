...
if ($User->check_level(MEMBER_LEVEL)) //Affichage des message()s non lu(s) du membre.
{
	...
	{
		$last_msg = $LANG['on'] . ' ' . gmdate_format('date_format', $row['last_timestamp']) . '<br /> ' . $LANG['by'] . ' <a class="small_link" href="'. DispatchManager::get_url('/member', '/profile/'. $row['last_user_id'] .'/')->absolute() .'">' . $row['last_login'] . '</a>';
	...
	$Template->put_all(array(
		...
		'LANG' => get_ulang(),
		'U_CHANGE_CAT'=> 'unread.php?token=' . $Session->get_token(),
