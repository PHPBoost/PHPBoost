...
if( !empty($CAT_FORUM[$id_get]['name']) ) //Nom de la catégorie courante.
	$speed_bar[$CAT_FORUM[$id_get]['name']] = '';
...
if( !empty($id_get) )
{
		...
		{	
			if( $groups->check_auth($CAT_FORUM[$row['cid']]['auth'], READ_CAT_FORUM) )
			{
				if( $row['nbr_msg'] !== '0' )
				{
					...
					$last_topic_title = (strlen(html_entity_decode($row['title'])) > 23) ? substr_html($row['title'], 0, 23) . '...' : $row['title'];			
					...
					<a href="topic' . transid('.php?' . $last_page .  'id=' . $row['tid'], '-' . $row['tid'] . $last_page_rewrite . '+' . url_encode_rewrite($row['title'])  . '.php') . '#m' .  $last_msg_id . '" title=""><img src="../templates/' . $CONFIG['theme'] . '/images/ancre.png" alt="" /></a> ' . $LANG['on'] . ' ' . date($LANG['date_format'] . ' ' . $LANG['at'] . ' H\hi', $row['last_timestamp']) . '<br />
	...
	$check_group_edit_auth = $groups->check_auth($CAT_FORUM[$id_get]['auth'], WRITE_CAT_FORUM);
	...
	$template->assign_vars(array(
		...
		'U_CHANGE_CAT'=> transid('.php?id=' . $id_get, '-' . $id_get . $rewrited_title . '.php'),
		...
		'U_POST_NEW_SUBJECT' => ($check_group_edit_auth && !$locked_cat) ? '&raquo; <a href="post' . transid('.php?new=topic&amp;id=' . $id_get, '') . '" title="' . $LANG['post_new_subject'] . '"><img class="valign_middle" src="../templates/' . $CONFIG['theme'] . '/images/' . $CONFIG['lang'] . '/post.png" alt="' . $LANG['post_new_subject'] . '" title="' . $LANG['post_new_subject'] . '" /></a>' : '',
	...
	list($total_admin, $total_modo, $total_member, $total_visit, $user_pseudo) = array(0, 0, 0, 0, '');
	...
	WHERE s.session_time > '" . (time() - $CONFIG['site_session_invit']) . "' AND s.session_script = '" . DIR . "/forum/forum.php' AND s.session_script_get LIKE '%id=" . $id_get . "%'
	...
	$template->assign_block_vars('online', array(
		'ONLINE' =>  ($total_online - $total_visit) == 0 ? '<em>' . $LANG['no_member_online'] . '</em>' : $user_pseudo 
	));
