...
while ( $row = $Sql->fetch_assoc($result) )
{
	...
	$Template->assign_block_vars('msg', array(
		...
		'USER_WEB' => (!empty($row['user_website'])) ? '<a href="' . $row['user_website'] . '"><img src="../templates/' . get_utheme() . '/images/' . get_ulang() . '/user_web.png" alt="' . $row['user_website']  . '" title="' . $row['user_website']  . '" /></a>' : '',
...
list($users_list, $total_admin, $total_modo, $total_member, $total_visit, $total_online) = forum_list_user_online("AND s.session_script LIKE '". GeneralConfig::get_default_site_path() . url('/forum/topic.php?id=' . $id_get, '/forum/topic-' . $id_get . $rewrited_title . '.php') ."'");
