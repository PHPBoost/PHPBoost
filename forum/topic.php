...
while ( $row = $Sql->fetch_assoc($result) )
{
	...
	$Template->assign_block_vars('msg', array(
		...
		'USER_PM' => !$is_guest ? '<a href="'. UserUrlBuilder::personnal_message($row['user_id'])->rel() . '" class="basic-button smaller">MP</a>' : '',
	));
...
list($users_list, $total_admin, $total_modo, $total_member, $total_visit, $total_online) = forum_list_user_online("AND s.session_script LIKE '%". GeneralConfig::get_default_site_path() . url('/forum/topic.php?id=' . $id_get, '/forum/topic-' . $id_get) ."%'");
...
else
{
	...
	if ($CONFIG_FORUM['activ_display_msg'] == 1 && ($check_group_edit_auth || $User->get_attribute('user_id') == $topic['user_id']))
	{
		...
		$Template->put_all(array(
			'C_DISPLAY_MSG' => true,
			'ICON_DISPLAY_MSG' => $CONFIG_FORUM['icon_activ_display_msg'] ? '<img src="' . PATH_TO_ROOT . '/forum/templates/images/' . $img_msg_display . '" alt="" class="valign_middle"  />' : '',
			'ICON_DISPLAY_MSG2' => $CONFIG_FORUM['icon_activ_display_msg'] ? '<img src="' . PATH_TO_ROOT . '/forum/templates/images/' . $img_msg_display . '" alt="" class="valign_middle" id="forum_change_img" />' : '',
