...
elseif( $session->check_auth($session->data, 0) ) //Affichage des message()s non lu(s) du membre.
{
	...
	LEFT JOIN ".PREFIX."member_extend AS me ON me.user_id = '" . $session->data['user_id'] . "'
	WHERE tr.user_id = '" . $session->data['user_id'] . "'
	...
	WHERE s.session_time > '" . (time() - $CONFIG['site_session_invit']) . "' AND s.session_script = '" . DIR . "/forum/track.php'
