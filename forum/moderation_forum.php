	...
	if (empty($id_get)) //On liste les alertes
	{
		...
		while ($row = $Sql->fetch_assoc($result))
		{
			...
			$Template->assign_block_vars('alert_list', array(
				...
				'EDIT' => '<a href="moderation_forum' . url('.php?action=alert&amp;id=' . $row['id']) . '" class="icon-edit"></a>',
	...
	if (empty($id_get)) //On liste les membres qui ont déjà un avertissement
	{
		...
		while ($row = $Sql->fetch_assoc($result))
		{
			...
			$Template->assign_block_vars('user_list', array(
				...
				'U_ACTION_USER' => '<a href="moderation_forum.php' . url('?action=punish&amp;id=' . $row['user_id'] . '&amp;token=' . $Session->get_token()) . '" class="icon-lock"></a>',
	...
	if (empty($id_get)) //On liste les membres qui ont déjà un avertissement
	{
		...
		while ($row = $Sql->fetch_assoc($result))
		{
			...
			$Template->assign_block_vars('user_list', array(
				...
				'U_ACTION_USER' => '<a href="moderation_forum.php' . url('?action=warning&amp;id=' . $row['user_id'] . '&amp;token=' . $Session->get_token()) . '"><img src="../forum/templates/images/important_mini.png" alt="" /></a>',
				'U_PROFILE' => UserUrlBuilder::profile($row['user_id'])->rel(),
