...
{			
	...
	{
		$switch_id_cat = $Sql->query("SELECT id FROM " . PREFIX . "forum_cats WHERE '" . $CAT_FORUM[$get_parent_up]['id_left'] . "' - id_right = 1");
		...
		{	
			...
			WHERE id_left < '" . $CAT_FORUM[$get_parent_up]['id_left'] . "' AND id_right > '" . $CAT_FORUM[$get_parent_up]['id_right'] . "'");
	...
	{
		$switch_id_cat = $Sql->query("SELECT id FROM " . PREFIX . "forum_cats WHERE id_left - '" . $CAT_FORUM[$get_parent_down]['id_right'] . "' = 1");
		if (!empty($switch_id_cat))
			echo $switch_id_cat;
		else
		{	
	...
	if (!empty($move) && !empty($id))
	{
		...
		if (array_key_exists($id, $CAT_FORUM))
		{
			...
			$result = $Sql->query_while("SELECT id 
			FROM " . PREFIX . "forum_cats 
			WHERE id_left < '" . $CAT_FORUM[$id]['id_left'] . "' AND id_right > '" . $CAT_FORUM[$id]['id_right'] . "'");
			...
			if (!empty($to)) //Changement de catégorie possible?
			{
				...
				if (!empty($list_parent_cats))
				{
					$max_timestamp_parent = $Sql->query("SELECT MAX(last_timestamp) FROM " . PREFIX . "forum_topics WHERE idcat IN (" . $list_parent_cats . ")");
					$max_topic_id_parent = $Sql->query("SELECT id FROM " . PREFIX . "forum_topics WHERE last_timestamp = '" . $max_timestamp_parent . "'"); 
				}
				...
				FROM " . PREFIX . "forum_cats 
				WHERE id_left <= '" . $CAT_FORUM[$to]['id_left'] . "' AND id_right >= '" . $CAT_FORUM[$to]['id_right'] . "'");
				...
				$max_timestamp = $Sql->query("SELECT MAX(last_timestamp) FROM " . PREFIX . "forum_topics WHERE idcat IN (" . $list_cats . ")");
				if (empty($list_parent_cats_to))
					$max_timestamp_to = $Sql->query("SELECT MAX(last_timestamp) FROM " . PREFIX . "forum_topics WHERE idcat = '" . $to . "'"); 
				else
					$max_timestamp_to = $Sql->query("SELECT MAX(last_timestamp) FROM " . PREFIX . "forum_topics WHERE idcat IN (" . $list_parent_cats_to . ")");
				
				$max_topic_id = $Sql->query("SELECT id FROM " . PREFIX . "forum_topics WHERE last_timestamp = '" . max($max_timestamp, $max_timestamp_to) . "'");
				...
				if (!empty($list_parent_cats))
				{
					$Sql->query_inject("UPDATE " . PREFIX . "forum_cats SET last_topic_id = '" . NumberHelper::numeric($max_topic_id_parent) . "', id_right = id_right - '" . ( $nbr_cat*2) . "' WHERE id IN (" . $list_parent_cats . ")");
				}
				...
				$Sql->query_inject("UPDATE " . PREFIX . "forum_cats SET last_topic_id = '" . NumberHelper::numeric($max_topic_id) . "' WHERE " . $clause_parent_cats_to);
