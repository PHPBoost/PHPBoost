...
if( !empty($view_msg) ) //Affichage de tous les messages du membre
{
	...
	LEFT JOIN ".PREFIX."forum_topics AS t ON msg.idtopic = t.id
	JOIN ".PREFIX."forum_cats AS c ON t.idcat = c.id AND c.aprob = 1" . $auth_cats . "
	...
	FROM ".PREFIX."forum_msg AS msg
	LEFT JOIN ".PREFIX."forum_topics AS t ON msg.idtopic = t.id
	JOIN ".PREFIX."forum_cats AS c ON t.idcat = c.id AND c.aprob = 1
	LEFT JOIN ".PREFIX."member AS m ON m.user_id = '" . $view_msg . "'
