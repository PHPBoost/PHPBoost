...
{	
	...
	function del_cat($idcat, $confirm_delete)
	{
		...
		if ($confirm_delete) //Confirmation de suppression, on supprime dans la bdd et on r�tabli l'arbre intervallaire.
		{
			...
			$Sql->query_inject("DELETE FROM " . PREFIX . "forum_topics WHERE idcat = '" . $idcat . "'", __LINE__, __FILE__); //On supprime les topics de la cat�gorie.
	...
	//R�cup�re la liste des parents d'une cat�gorie.
