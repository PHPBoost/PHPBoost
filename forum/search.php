	...
	if (strlen($search) >= 4)
	{
		...
		{ 
			...
			$Template->assign_block_vars('list', array(
				'USER_ONLINE' => '<i class="' . ((!empty($row['connect']) && $row['user_id'] !== -1) ? 'icon-online' : 'icon-offline') . '"></i>',
...
list($users_list, $total_admin, $total_modo, $total_member, $total_visit, $total_online) = forum_list_user_online("AND s.session_script LIKE '%". GeneralConfig::get_default_site_path() ."/forum/search.php%'");
