...
while ($row = $Sql->fetch_assoc($result))
{
	...
	else //On liste les sous-catégories
	{
		...
		if (!empty($row['last_topic_id']))
		{
			...
			<a href="topic' . url('.php?' . $last_page .  'id=' . $row['tid'], '-' . $row['tid'] . $last_page_rewrite . '+' . Url::encode_rewrite($row['title'])  . '.php') . '#m' .  $last_msg_id . '"><img src="../templates/' . get_utheme() . '/images/ancre.png" alt="" /></a> ' . $LANG['on'] . ' ' . gmdate_format('date_format', $row['last_timestamp']) . '<br />' . $LANG['by'] . ' ' . ($row['last_user_id'] != '-1' ? '<a href="'. DispatchManager::get_url('/member', '/profile/'.$row['last_user_id'])->absolute() . '" class="small_link">' . $row['login'] . '</a>' : '<em>' . $LANG['guest'] . '</em>');
		}
		else
		{
			$row['last_timestamp'] = '';
