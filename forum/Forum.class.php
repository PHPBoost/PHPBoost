...
class Forum
{
	...
	function Add_msg($idtopic, $idcat, $contents, $title, $last_page, $last_page_rewrite, $new_topic = false)
	{
		...
		##### Insertion message #####
		$last_timestamp = time();
		$Sql->query_inject("INSERT INTO " . PREFIX . "forum_msg (idtopic, user_id, contents, timestamp, timestamp_edit, user_id_edit, user_ip) VALUES ('" . $idtopic . "', '" . $User->get_id() . "', '" . FormatingHelper::strparse($contents) . "', '" . $last_timestamp . "', '0', '0', '" . USER_IP . "')", __LINE__, __FILE__);
		$last_msg_id = $Sql->insert_id("SELECT MAX(id) FROM " . PREFIX . "forum_msg");

		//Topic
		$Sql->query_inject("UPDATE " . PREFIX . "forum_topics SET " . ($new_topic ? '' : 'nbr_msg = nbr_msg + 1, ') . "last_user_id = '" . $User->get_id() . "', last_msg_id = '" . $last_msg_id . "', last_timestamp = '" . $last_timestamp . "' WHERE id = '" . $idtopic . "'", __LINE__, __FILE__);
		...
		$Sql->query_inject("UPDATE " . DB_TABLE_MEMBER . " SET user_msg = user_msg + 1 WHERE user_id = '" . $User->get_id() . "'", __LINE__, __FILE__);
		...
		if (!$new_topic)
		{
			...
			if ($User->get_id() > 0)
			{
				$pseudo = $Sql->query("SELECT login FROM " . DB_TABLE_MEMBER . " WHERE user_id = '" . $User->get_id() . "'", __LINE__, __FILE__);
				$pseudo_pm = '[url='. DispatchManager::get_url('/member', '/profile/'. $User->get_id() .'/')->absolute() .']' . $pseudo . '[/url]';
			}
			else
			{
				$pseudo = $LANG['guest'];
			...
			WHERE tr.idtopic = '" . $idtopic . "' AND v.last_view_id IS NOT NULL AND m.user_id != '" . $User->get_id() . "'", __LINE__, __FILE__);
	...
	function Add_topic($idcat, $title, $subtitle, $contents, $type)
	{
		...
		$Sql->query_inject("INSERT INTO " . PREFIX . "forum_topics (idcat, title, subtitle, user_id, nbr_msg, nbr_views, last_user_id, last_msg_id, last_timestamp, first_msg_id, type, status, aprob, display_msg) VALUES ('" . $idcat . "', '" . $title . "', '" . $subtitle . "', '" . $User->get_id() . "', 1, 0, '" . $User->get_id() . "', '0', '" . time() . "', 0, '" . $type . "', 1, 0, 0)", __LINE__, __FILE__);
	...
	function Update_msg($idtopic, $idmsg, $contents, $user_id_msg, $history = true)
	{
		...
		$edit_mark = (!$User->check_auth($CONFIG_FORUM['auth'], EDIT_MARK_FORUM)) ? ", timestamp_edit = '" . time() . "', user_id_edit = '" . $User->get_id() . "'" : '';
		...
		if ($User->get_id() != $user_id_msg && $history)
	...
	function Track_topic($idtopic, $tracking_type = 0)
	{
		...
		$exist = $Sql->query("SELECT COUNT(*) FROM " . PREFIX . "forum_track WHERE user_id = '" . $User->get_id() . "' AND idtopic = '" . $idtopic . "'", __LINE__, __FILE__);
		if ($exist == 0)
			$Sql->query_inject("INSERT INTO " . PREFIX . "forum_track (idtopic, user_id, track, pm, mail) VALUES('" . $idtopic . "', '" . $User->get_id() . "', '" . $track . "', '" . $pm . "', '" . $mail . "')", __LINE__, __FILE__);
		elseif ($tracking_type == 0)
			$Sql->query_inject("UPDATE " . PREFIX . "forum_track SET track = '1' WHERE idtopic = '" . $idtopic . "' AND user_id = '" . $User->get_id() . "'", __LINE__, __FILE__);
		elseif ($tracking_type == 1)
			$Sql->query_inject("UPDATE " . PREFIX . "forum_track SET mail = '1' WHERE idtopic = '" . $idtopic . "' AND user_id = '" . $User->get_id() . "'", __LINE__, __FILE__);
		elseif ($tracking_type == 2)
			$Sql->query_inject("UPDATE " . PREFIX . "forum_track SET pm = '1' WHERE idtopic = '" . $idtopic . "' AND user_id = '" . $User->get_id() . "'", __LINE__, __FILE__);
		...
		if (!$User->check_auth($CONFIG_FORUM['auth'], TRACK_TOPIC_FORUM))
		{
			...
			$Sql->query_inject("DELETE FROM " . PREFIX . "forum_track WHERE user_id = '" . $User->get_id() . "' AND id < @compt", __LINE__, __FILE__);
		}
	}
	...
	function Untrack_topic($idtopic, $tracking_type = 0)
	{
		...
		{
			$info = $Sql->query_array(PREFIX . "forum_track", "pm", "track", "WHERE user_id = '" . $User->get_id() . "' AND idtopic = '" . $idtopic . "'", __LINE__, __FILE__);
			if ($info['track'] == 0 && $info['pm'] == 0)
				$Sql->query_inject("DELETE FROM " . PREFIX . "forum_track WHERE idtopic = '" . $idtopic . "' AND user_id = '" . $User->get_id() . "'", __LINE__, __FILE__);
			else
				$Sql->query_inject("UPDATE " . PREFIX . "forum_track SET mail = '0' WHERE idtopic = '" . $idtopic . "' AND user_id = '" . $User->get_id() . "'", __LINE__, __FILE__);
		}
		elseif ($tracking_type == 2) //Par mp
		{
			$info = $Sql->query_array(PREFIX . "forum_track", "mail", "track", "WHERE user_id = '" . $User->get_id() . "' AND idtopic = '" . $idtopic . "'", __LINE__, __FILE__);
			if ($info['mail'] == 0 && $info['track'] == 0)
				$Sql->query_inject("DELETE FROM " . PREFIX . "forum_track WHERE idtopic = '" . $idtopic . "' AND user_id = '" . $User->get_id() . "'", __LINE__, __FILE__);
			else
				$Sql->query_inject("UPDATE " . PREFIX . "forum_track SET pm = '0' WHERE idtopic = '" . $idtopic . "' AND user_id = '" . $User->get_id() . "'", __LINE__, __FILE__);
		}
		else //Suivi
		{
			$info = $Sql->query_array(PREFIX . "forum_track", "mail", "pm", "WHERE user_id = '" . $User->get_id() . "' AND idtopic = '" . $idtopic . "'", __LINE__, __FILE__);
			if ($info['mail'] == 0 && $info['pm'] == 0)
				$Sql->query_inject("DELETE FROM " . PREFIX . "forum_track WHERE idtopic = '" . $idtopic . "' AND user_id = '" . $User->get_id() . "'", __LINE__, __FILE__);
			else
				$Sql->query_inject("UPDATE " . PREFIX . "forum_track SET track = '0' WHERE idtopic = '" . $idtopic . "' AND user_id = '" . $User->get_id() . "'", __LINE__, __FILE__);
		}
	}
	...
	function Alert_topic($alert_post, $alert_title, $alert_contents)
	{
		...
		$Sql->query_inject("INSERT INTO " . PREFIX . "forum_alerts (idcat, idtopic, title, contents, user_id, status, idmodo, timestamp) VALUES ('" . $topic_infos['idcat'] . "', '" . $alert_post . "', '" . $alert_title . "', '" . $alert_contents . "', '" . $User->get_id() . "', 0, 0, '" . time() . "')", __LINE__, __FILE__);
	...
	function Solve_alert_topic($id_alert)
	{
		...
		$Sql->query_inject("UPDATE " . PREFIX . "forum_alerts SET status = 1, idmodo = '" . $User->get_id() . "' WHERE id = '" . $id_alert . "'", __LINE__, __FILE__);
