...
else if (isset($_GET['edit_category']) && is_numeric($id)) // edition d'une cat�gorie
{
	$Template->set_filenames(array(
		'admin_bugtracker'=> 'bugtracker/admin_bugtracker.tpl'
	));
...
else if (isset($_GET['delete_category']) && is_numeric($id)) //Suppression d'une cat�gorie
{
	$Session->csrf_get_protect(); //Protection csrf
	
	//Suppression de la cat�gorie en question dans la liste des bugs
	$Sql->query_inject("UPDATE " . PREFIX . "bugtracker SET category = '' WHERE category = '" . addslashes($categories[$id]) . "'", __LINE__, __FILE__);
	
	//On supprime la cat�gorie de la liste
	unset($categories[$id]);
