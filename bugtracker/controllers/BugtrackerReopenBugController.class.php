...
class BugtrackerReopenBugController extends ModuleController
{
	public function execute(HTTPRequestCustom $request)
	{
		...
		if (!$bug->is_reopen())
		{
			...
			{
				case 'detail' :
					$redirect = BugtrackerUrlBuilder::detail_success('reopen/'. $id);
					break;
				default :
					$redirect = BugtrackerUrlBuilder::solved_success('reopen/'. $id . '/' . $page . (!empty($back_filter) ? '/' . $back_filter . '/' . $filter_id : ''));
					break;
			}
