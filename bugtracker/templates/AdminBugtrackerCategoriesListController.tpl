...
BugtrackerFormFieldCategories.prototype = {
	add_category : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<a/> ', {id : 'delete_' + id, onclick : 'BugtrackerFormFieldCategories.delete_category(' + id + ');return false;', 'aria-label' : ${escapejs(@titles.del_category)}}).html('<i class="fa fa-delete" aria-hidden="true" title="${@titles.del_category}"></i>').appendTo('#td3_' + id);
...
<table class="table categories-list">
	...
	<tbody id="categories_list">
		...
		<tr>
			...
			<td>
				<a href="{categories.LINK_DELETE}" aria-label="${@titles.del_category}" data-confirmation="delete-element"><i class="fa fa-delete" aria-hidden="true" title="${@titles.del_category}"></i></a>
			</td>
		</tr>
		# END categories #
	</tbody>
	<tfoot>
		<tr>
			<td>
				<a href="" onclick="BugtrackerFormFieldCategories.add_category();return false;" aria-label="{@titles.add_category}" id="add-category"><i class="fa fa-plus" aria-hidden="true" title="{@titles.add_category}"></i></a>
			</td>
