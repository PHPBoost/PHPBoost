...
BugtrackerFormFieldTypes.prototype = {
	add_type : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<div/>', {id : 'types_radio_' + id, class: 'form-field-radio custom-radio'}).appendTo('#td1_' + id);
			...
			jQuery('<a/> ', {id : 'delete_' + id, onclick : 'BugtrackerFormFieldTypes.delete_type(' + id + ');return false;', 'aria-label' : ${escapejs(@titles.del_type)}}).html('<i class="fa fa-delete" aria-hidden="true"></i>').appendTo('#td3_' + id);
...
<table class="table type-list">
	...
	<tbody id="types_list">
		...
		<tr>
			<td>
				<div id="types_radio_{types.ID}" class="form-field-radio custom-radio">
			...
			<td>
				<a href="{types.LINK_DELETE}" aria-label="${@titles.del_type}" data-confirmation="delete-element"><i class="fa fa-delete" aria-hidden="true"></i></a>
			</td>
		</tr>
		# END types #
	</tbody>
