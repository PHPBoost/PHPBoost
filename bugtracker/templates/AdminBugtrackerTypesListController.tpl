...
BugtrackerFormFieldTypes.prototype = {
	add_type : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<a/> ', {id : 'delete_' + id, onclick : 'BugtrackerFormFieldTypes.delete_type(' + id + ');return false;', 'aria-label' : ${escapejs(@titles.del_type)}}).html('<i class="fa fa-delete" aria-hidden="true" title="${@titles.del_type}"></i>').appendTo('#td3_' + id);
...
<table class="table type-list">
	...
	<tbody id="types_list">
		...
		<tr>
			...
			<td>
				<a href="{types.LINK_DELETE}" aria-label="${@titles.del_type}" data-confirmation="delete-element"><i class="fa fa-delete" aria-hidden="true" title="${@titles.del_type}"></i></a>
			</td>
		</tr>
		# END types #
	</tbody>
		...
		<tr>
			<td colspan="3">
				<div class="cw25 float-left">
					<a href="" onclick="BugtrackerFormFieldTypes.add_type();return false;" aria-label="{@titles.add_type}" id="add-type"><i class="fa fa-plus" aria-hidden="true" title="{@titles.add_type}"></i></a>
				</div>
