...
{	
	...
	$Template->assign_vars(array(
		...
		'L_CONTENT_EXPLAIN'			=> $LANG['bugs.explain.contents'],
...
else if (isset($_GET['roadmap'])) // roadmap
{
	...
	$Template->assign_vars(array(
		...
		'NO_BUGS_COLSPAN' 		=> $no_bugs_colspan,
		'C_COM' 				=> ($comments_activated == true) ? true : false,
		'PAGINATION' 			=> $Pagination->display(PATH_TO_ROOT . '/bugtracker/bugtracker' . url('.php?roadmap&amp;roadmap_version=' . $r_version . '&amp;p=%d' . (!empty($get_sort) ? '&amp;sort=' . $get_sort : '') . (!empty($get_mode) ? '&amp;mode=' . $get_mode : '')), $nbr_bugs, 'p', $items_per_page, 3),
		'L_ROADMAP' 			=> $LANG['bugs.titles.roadmap'],
