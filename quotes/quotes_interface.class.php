...
class QuotesInterface extends ModuleInterface
{
	...
	*/
	function get_cache()
	{
		...
		$rand_limit = numeric(array('left', 'right')) ? array('left', 'right') : 1;
		
		$quotes_code .= "\n\n" . 'global $_quotes_rand_msg;' . "\n";
		$quotes_code .= "\n" . '$_quotes_rand_msg = array();' . "\n";
		$result = $Sql->query_while(
			"SELECT q.id, q.user_id, q.timestamp, q.contents, q.author,
			m.login AS mlogin
			FROM ".PREFIX."quotes q
			LEFT JOIN ".PREFIX."member m ON m.user_id = q.user_id
			ORDER BY q.timestamp DESC"
			. $Sql->limit(0, $rand_limit), __LINE__, __FILE__);	
		while ($row = $Sql->fetch_assoc($result))
		{
			$quotes_code .= '$_quotes_rand_msg[] = array(\'id\' => ' . var_export($row['id'], true) . ', \'author\' => ' . var_export(substr_html(strip_tags($row['author']), 0), true) . ', \'contents\' => ' . var_export(substr_html(strip_tags($row['contents']), 0), true) . ', \'user_id\' => ' . var_export($row['user_id'], true) . ');' . "\n";
		}
