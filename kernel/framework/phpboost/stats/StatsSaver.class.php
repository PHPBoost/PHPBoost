	...
	public static function compute_referer()
	{
		...
		if (!empty($referer))
		{
			...
			if ($is_search_engine)
			{
				...
				$check_search_engine = PersistenceContext::get_sql()->query("SELECT COUNT(*) FROM " . DB_TABLE_STATS_REFERER . " WHERE url = '" . $search_engine . "' AND relative_url = '" . $keyword . "'", __LINE__, __FILE__);
				if (!empty($keyword))
				{
					if (!empty($check_search_engine))
						PersistenceContext::get_sql()->query_inject("UPDATE " . DB_TABLE_STATS_REFERER . " SET total_visit = total_visit + 1, today_visit = today_visit + 1, last_update = '" . time() . "' WHERE url = '" . $search_engine . "' AND relative_url = '" . $keyword . "'", __LINE__, __FILE__);			
					else
						PersistenceContext::get_sql()->query_inject("INSERT INTO " . DB_TABLE_STATS_REFERER . " (url, relative_url, total_visit, today_visit, yesterday_visit, nbr_day, last_update, type) VALUES ('" . $search_engine . "', '" . $keyword . "', 1, 1, 1, 1, '" . time() . "', 1)", __LINE__, __FILE__);
				}
			}
			elseif (!empty($referer['host']))
			{
				...
				{				
					...
					$check_url = PersistenceContext::get_sql()->query("SELECT COUNT(*) FROM " . DB_TABLE_STATS_REFERER . " WHERE url = '" . $url . "' AND relative_url = '" . $relative_url . "'", __LINE__, __FILE__);
					if (!empty($check_url))
						PersistenceContext::get_sql()->query_inject("UPDATE " . DB_TABLE_STATS_REFERER . " SET total_visit = total_visit + 1, today_visit = today_visit + 1, last_update = '" . time() . "' WHERE url = '" . $url . "' AND relative_url = '" . $relative_url . "'", __LINE__, __FILE__);			
					else
						PersistenceContext::get_sql()->query_inject("INSERT INTO " . DB_TABLE_STATS_REFERER . " (url, relative_url, total_visit, today_visit, yesterday_visit, nbr_day, last_update, type) VALUES ('" . $url . "', '" . $relative_url . "', 1, 1, 1, 1, '" . time() . "', 0)", __LINE__, __FILE__);
				}
			}
		}
	}
