...
class ModuleConfiguration
{
	...
	private function load_configuration($config_ini_file)
	{
		...
		$this->features               = isset($config['features']) ? (!empty($config['features']) ? explode(',', preg_replace('/\s/', '', $config['features'])) : array()) : array();
		$this->specific_hooks         = isset($config['specific_hooks']) ? (!empty($config['specific_hooks']) ? explode(',', preg_replace('/\s/', '', $config['specific_hooks'])) : array()) : array();
		...
		$this->url_rewrite_rules      = isset($config['rewrite_rules']) ? (!empty($config['rewrite_rules']) ? $config['rewrite_rules'] : array()) : array();
