...
class FormFieldAjaxCompleter extends FormFieldTextEditor
{
	...
	class="${escape(CLASS)}" # IF C_DISABLED # disabled="disabled" # ENDIF # autocomplete="off" onfocus="javascript:load_autocompleter_{ID}();" /><div id="${escape(ID)}_completer" class="form_autocompleter"></div>
	<script type="text/javascript">function load_autocompleter_{ID}() { new Ajax.Autocompleter(${escapejs(ID)}, "${escape(ID)}_completer", ${escapejs(FILE)}, { method: ${escapejs(METHOD)}, paramName: ${escapejs(NAME_PARAMETER)}});}</script>';	
