	...
	public function get_search_request($args)
	{
		...
		if ($this->read_authorization !== false)
		{
			return "SELECT " . $args['id_search'] . " AS id_search,
				...
				" . $table_label . "." . $this->field_title . " AS title,
				( 2 * FT_SEARCH_RELEVANCE(" . $table_label . "." . $this->field_title . ", '" . $args['search'] . "' IN BOOLEAN MODE) + (FT_SEARCH_RELEVANCE(" . $table_label . "." . $this->field_contents . ", '" . $args['search'] . "' IN BOOLEAN MODE)" . ($this->has_short_contents ? " +
				FT_SEARCH_RELEVANCE(" . $table_label . "." . $this->field_short_contents . ", '" . $args['search'] . "' IN BOOLEAN MODE)) / 2 " : ")") . ") / 3 * " . $weight . " AS relevance,
				...
				" . ($this->cats_table_name ? "LEFT JOIN " . $this->cats_table_name . " cat ON " . $table_label . ".id_category = cat.id" : "") . "
				...
				WHERE ( FT_SEARCH(" . $table_label . "." . $this->field_title . ", '" . $args['search'] . "*' IN BOOLEAN MODE) OR FT_SEARCH(" . $table_label . "." . $this->field_contents . ", '" . $args['search'] . "*' IN BOOLEAN MODE)" . ($this->has_short_contents ? " OR FT_SEARCH_RELEVANCE(" . $table_label . "." . $this->field_short_contents . ", '" . $args['search'] . "*' IN BOOLEAN MODE)" : "") . " )" . ($this->use_keywords ? " OR keyword.rewrited_name = '" . Url::encode_rewrite($args['search']) . "'" : "") . "
