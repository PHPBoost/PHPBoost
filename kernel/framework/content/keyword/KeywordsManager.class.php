...
class KeywordsManager
{
	...
	public function put_relations($id_in_module, $keywords)
	{
		...
		foreach ($keywords as $keyword)
		{
			if (!empty($keyword))
			{
				...
				{
					$result = $this->db_querier->insert(DB_TABLE_KEYWORDS, array('name' => TextHelper::htmlspecialchars($keyword), 'rewrited_name' => Url::encode_rewrite(TextHelper::htmlentities($keyword))));
					$id_keyword = $result->get_last_inserted_id();
				}
				else
				{
					$id_keyword = $this->db_querier->get_column_value(DB_TABLE_KEYWORDS, 'id', 'WHERE rewrited_name=:rewrited_name', array('rewrited_name' => Url::encode_rewrite(TextHelper::htmlentities($keyword))));
				}
	...
	{
		return $this->db_querier->row_exists(DB_TABLE_KEYWORDS, 'WHERE rewrited_name=:rewrited_name', array('rewrited_name' => Url::encode_rewrite(TextHelper::htmlentities($name))));
	}
}
?>