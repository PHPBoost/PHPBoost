...
class DefaultItemsManagementController extends AbstractItemController
{
	...
	private function build_table()
	{
		...
		foreach ($result as $row)
		{
			...
			if ($display_categories)
				array_splice($row, 1, 0, array(new HTMLTableRowCell(new LinkHTMLElement(CategoriesUrlBuilder::display_category($category->get_id(), $category->get_rewrited_name()), ($category->get_id() == Category::ROOT_CATEGORY ? LangLoader::get_message('none_e', 'common') : $category->get_name())))));
