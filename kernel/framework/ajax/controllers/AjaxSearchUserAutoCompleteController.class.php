	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		try {
			...
			while($row = $result->fetch())
			{
				...
				if ($is_admin)
				{
					$edit_link = new LinkHTMLElement(UserUrlBuilder::edit_profile($row['user_id']), '', array('title' => $lang['edit']), 'far fa-edit');
					
					if ($row['level'] != User::ADMIN_LEVEL || ($row['level'] == User::ADMIN_LEVEL && $number_admins > 1))
						$delete_link = new LinkHTMLElement(AdminMembersUrlBuilder::delete($row['user_id']), '', array('title' => $lang['delete'], 'data-confirmation' => 'delete-element'), 'far fa-delete');
					else
						$delete_link = new LinkHTMLElement('', '', array('title' => $lang['delete'], 'onclick' => 'return false;'), 'far fa-delete icon-disabled');
