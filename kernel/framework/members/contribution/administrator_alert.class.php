...
class AdministratorAlert extends Contribution 
{
	...
	function load_from_db($id_contrib)
	{
		...
		$result = $Sql->Query_while("SELECT id, entitled, fixing_url, module, current_status, id_in_module, identifier, type, priority, creation_date, fixing_date, auth, poster_id, fixer_id, poster_member.login poster_login, fixer_member.login fixer_login, description
	...
	function build_from_db($id, $entitled, $description, $fixing_url, $module, $current_status, $creation_date, $fixing_date, $auth, $poster_id, $fixer_id, $id_in_module, $identifier, $type, $priority)
	{
		parent::build_from_db($id, $entitled, $description, $fixing_url, $module, $current_status, $creation_date, $fixing_date, $auth, $poster_id, $fixer_id, $id_in_module, $identifier, $type);
	...
	function save()
	{
		...
		if( $this->id > 0 )
		{
			...
			$Sql->Query_inject("UPDATE ".PREFIX."contributions SET entitled = '" . addslashes($this->entitled) . "', description = '" . addslashes($this->description) . "', fixing_url = '" . addslashes($this->fixing_url) . "', module = '" . addslashes($this->module) . "', current_status = '" . $this->current_status . "', creation_date = '" . $creation_date->get_timestamp() . "', fixing_date = '" . $fixing_date->get_timestamp() . "', auth = '" . addslashes(serialize($this->auth)) . "', poster_id = '" . $this->poster_id . "', fixer_id = '" . $this->fixer_id . "', id_in_module = '" . $this->id_in_module . "', identifier = '" . addslashes($this->identifier) . "', type = '" . addslashes($this->type) . "', priority = '" . $this->priority . "' WHERE id = '" . $this->id . "'", __LINE__, __FILE__);
		}
		else //We create it
		{
			$this->creation_date = new Date();
			$Sql->Query_inject("INSERT INTO ".PREFIX."contributions (entitled, description, fixing_url, module, current_status, creation_date, fixing_date, auth, poster_id, fixer_id, id_in_module, identifier, type, priority) VALUES ('" . addslashes($this->entitled) . "', '" . addslashes($this->description) . "', '" . addslashes($this->fixing_url) . "', '" . addslashes($this->module) . "', '" . $this->current_status . "', '" . $this->creation_date->get_timestamp() . "', 0, '" . (!empty($this->auth) ? addslashes(serialize($this->auth)) : '') . "', '" . $this->poster_id . "', '" . $this->fixer_id . "', '" . $this->id_in_module . "', '" . addslashes($this->identifier) . "', '" . addslashes($this->type) . "', '" . $this->priority . "')", __LINE__, __FILE__);
