...
//Création de l'objet qui gère les requêtes SQ
$Sql = new Sql();
...
if (gmdate_format('j', time(), TIMEZONE_SITE) != $_record_day && !empty($_record_day))
{
    ...
    $check_update = (int) $Sql->query("SELECT COUNT(*) 
	FROM " . PREFIX . "stats
	...
	stats_day = '" . gmdate_format('d', $yesterday_timestamp, TIMEZONE_SYSTEM) . "'", __LINE__, __FILE__	);
