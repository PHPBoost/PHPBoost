...
//Class de la gallery, plus héritage de la class sql pour les requêtes necessaire dans la class.
class gallery extends Sql
{	
	...
	function resize_pics($path, $width_max = 0, $height_max = 0)
	{
		...
		{	
			...
			else
			{
				...
				{	
					$thumbtail = @imagecreatetruecolor($width, $height);
					if( $thumbtail === false )				
						$this->error = 'e_unabled_create_pics';
				}
				...
				{	
					if( @imagecopyresized($thumbtail, $source, 0, 0, 0, 0, $width, $height, $width_s, $height_s) === false )				
						$this->error = 'e_error_resize';
				}
				else
				{	
					if( @imagecopyresampled($thumbtail, $source, 0, 0, 0, 0, $width, $height, $width_s, $height_s) === false )				
						$this->error = 'e_error_resize';
				}
			}
			...
			//Création de l'image.
			if( empty($this->error) )
				$this->create_pics($thumbtail, $source, $path, $ext);
		}
		else
		{
			$path_mini = str_replace('pics', 'pics/thumbnails', $path);
	...
	//Création de l'image.
	function create_pics($thumbtail, $source, $path, $ext)
	{
		global $CONFIG_GALLERY;
		
		$path_mini = str_replace('pics', 'pics/thumbnails', $path);
		if( function_exists('imagegif') && $ext === 'gif' ) 
			imagegif($thumbtail, $path_mini);
		elseif( function_exists('imagejpeg') && $ext === 'jpg' ) 
			imagejpeg($thumbtail, $path_mini, $CONFIG_GALLERY['quality']);
		elseif( function_exists('imagepng')  && $ext === 'png' ) 
			imagepng($thumbtail, $path_mini);
		else 
	...
	function create_pics_error($path, $width, $height)
	{
		...
		//Ecriture du code.
		imagettftext($thumbtail, $font_size, 0, $text_x, $text_y, $text_color, $font, $LANG['e_error_img']);
		@imagejpeg($thumbtail, $path, 75);
	}
	...
	function add_pics($idcat, $name, $path, $user_id)
	{
		...
		parent::query_inject("UPDATE ".PREFIX."gallery_cats SET nbr_pics_aprob = nbr_pics_aprob + 1 WHERE " . $clause_parent_cats_to, __LINE__, __FILE__);		
		
		list($width, $height, $weight, $ext) = $this->arg_pics('pics/' . $path);	
		parent::query_inject("INSERT INTO ".PREFIX."gallery (idcat, name, path, width, height, weight, user_id, aprob, views, timestamp, users_note, nbrnote, note, nbr_com) VALUES('" . $idcat . "', '" . $name . "', '" . $path . "', '" . $width . "', '" . $height . "', '" . $weight ."', '" . $user_id . "', 1, 0, '" . time() . "', '', 0, 0, 0)", __LINE__, __FILE__);
		
		return parent::sql_insert_id("SELECT MAX(id) FROM ".PREFIX."gallery");
	}
	...
	function del_pics($id_pics)
	{
		...
		$info_pics = parent::query_array("gallery", "path", "idcat", "aprob", "WHERE id = '" . $id_pics . "'", __LINE__, __FILE__);
		if( !empty($info_pics['path']) )
		{
			parent::query_inject("DELETE FROM ".PREFIX."gallery WHERE id = '" . $id_pics . "'", __LINE__, __FILE__);	
			...
			if( $info_pics['aprob'] )
				...
				parent::query_inject("UPDATE ".PREFIX."gallery_cats SET nbr_pics_unaprob = nbr_pics_unaprob - 1 WHERE " . $clause_parent_cats_to, __LINE__, __FILE__);
		}
	...
	{
		parent::query_inject("UPDATE ".PREFIX."gallery SET name = '" . $name . "' WHERE id = '" . $id_pics . "'", __LINE__, __FILE__);
	...
	function aprob_pics($id_pics)
	{
		...
		$idcat = parent::query("SELECT idcat FROM ".PREFIX."gallery WHERE id = '" . $id_pics . "'", __LINE__, __FILE__);
		//Parent de la catégorie cible
		$list_parent_cats_to = '';
		$result = parent::query_while("SELECT id 
		FROM ".PREFIX."gallery_cats 
		...
		$aprob = parent::query("SELECT aprob FROM ".PREFIX."gallery WHERE id = '" . $id_pics . "'", __LINE__, __FILE__);
		if( $aprob )
		{	
			parent::query_inject("UPDATE ".PREFIX."gallery SET aprob = 0 WHERE id = '" . $id_pics . "'", __LINE__, __FILE__);
			parent::query_inject("UPDATE ".PREFIX."gallery_cats SET nbr_pics_unaprob = nbr_pics_unaprob + 1, nbr_pics_aprob = nbr_pics_aprob - 1 WHERE " . $clause_parent_cats_to, __LINE__, __FILE__);
		}
		else
		{
			parent::query_inject("UPDATE ".PREFIX."gallery SET aprob = 1 WHERE id = '" . $id_pics . "'", __LINE__, __FILE__);
			parent::query_inject("UPDATE ".PREFIX."gallery_cats SET nbr_pics_unaprob = nbr_pics_unaprob - 1, nbr_pics_aprob = nbr_pics_aprob + 1 WHERE " . $clause_parent_cats_to, __LINE__, __FILE__);
		}
	...
	function move_pics($id_pics, $id_move)
	{
		...
		if( $aprob )
		{	
			parent::query_inject("UPDATE ".PREFIX."gallery_cats SET nbr_pics_aprob = nbr_pics_aprob - 1 WHERE " . $clause_parent_cats, __LINE__, __FILE__);
			parent::query_inject("UPDATE ".PREFIX."gallery_cats SET nbr_pics_aprob = nbr_pics_aprob + 1 WHERE " . $clause_parent_cats_to, __LINE__, __FILE__);
		}
		else
		{
			parent::query_inject("UPDATE ".PREFIX."gallery_cats SET nbr_pics_unaprob = nbr_pics_unaprob - 1 WHERE " . $clause_parent_cats, __LINE__, __FILE__);
			parent::query_inject("UPDATE ".PREFIX."gallery_cats SET nbr_pics_unaprob = nbr_pics_unaprob + 1 WHERE " . $clause_parent_cats_to, __LINE__, __FILE__);
		}
		parent::query_inject("UPDATE ".PREFIX."gallery SET idcat = '" . $id_move . "' WHERE id = '" . $id_pics . "'", __LINE__, __FILE__);
	}
	
	//Note une image.
	function note_pics($id_pics, $note, $user_id)
	{
		$info_pics = parent::query_array('gallery', 'id', 'users_note', 'nbrnote', 'note', "WHERE id = '" . $id_pics . "'", __LINE__, __FILE__);
		...
		{			
			...
			parent::query_inject("UPDATE ".PREFIX."gallery SET note = '" . $note . "', nbrnote = '" . ($info_pics['nbrnote'] + 1) . "', users_note = '" . $users_note . "' WHERE id = '" . $id_pics . "'", __LINE__, __FILE__);
	...
	{
		return parent::query("SELECT COUNT(*) FROM ".PREFIX."gallery WHERE user_id = '" . $user_id . "'", __LINE__, __FILE__);
	}
	...
	function count_cat_pics()
	{
		...
		$info_cat = array();
		$result = parent::query_while("SELECT idcat, COUNT(*) as nbr_pics_aprob 
		FROM ".PREFIX."gallery 
		...
		$result = parent::query_while("SELECT idcat, COUNT(*) as nbr_pics_unaprob 
		FROM ".PREFIX."gallery 
		...
		$result = parent::query_while("SELECT id, id_left, id_right
		...
		{			
			...
			parent::query_inject("UPDATE ".PREFIX."gallery_cats SET nbr_pics_aprob = '" . $nbr_pics_aprob . "', nbr_pics_unaprob = '" . $nbr_pics_unaprob . "' WHERE id = '" . $row['id'] . "'", __LINE__, __FILE__);	
		}
		parent::close($result);
	}
