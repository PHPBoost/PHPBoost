...
class GalleryInterface extends ModuleInterface
{
	...
	function get_cache()
	{
		...
		$CONFIG_GALLERY = unserialize($this->db_connection->query("SELECT value FROM " . DB_TABLE_CONFIGS . " WHERE name = 'gallery'", __LINE__, __FILE__));
		...
		$result = $this->db_connection->query_while("SELECT id, id_left, id_right, level, name, aprob, auth
		...
		$result = $this->db_connection->query_while("SELECT g.id, g.name, g.path, g.width, g.height, g.idcat, gc.auth 
		FROM " . PREFIX . "gallery g
		LEFT JOIN " . PREFIX . "gallery_cats gc on gc.id = g.idcat
		WHERE g.aprob = 1 AND (gc.aprob = 1 OR g.idcat = 0)
		ORDER BY RAND()
		" . $this->db_connection->limit(0, 30), __LINE__, __FILE__);
		while ($row = $this->db_connection->fetch_assoc($result))
		{
			if ($row['idcat'] == 0)
