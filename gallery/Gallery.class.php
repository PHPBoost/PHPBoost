...
{	
	...
	public function Add_pics($idcat, $name, $path, $user_id)
	{
		...
		$Sql->query_inject("INSERT INTO " . PREFIX . "gallery (idcat, name, path, width, height, weight, user_id, aprob, views, timestamp, users_note, nbrnote, note, nbr_com) VALUES('" . $idcat . "', '" .TextHelper::strprotect($name,HTML_PROTECT,ADDSLASHES_FORCE). "', '" . $path . "', '" . $width . "', '" . $height . "', '" . $weight ."', '" . $user_id . "', 1, 0, '" . time() . "', '', 0, 0, 0)", __LINE__, __FILE__);
	...
	public function Rename_pics($id_pics, $name, $previous_name)
	{
		...
		$Sql->query_inject("UPDATE " . PREFIX . "gallery SET name = '" . TextHelper::strprotect($name,HTML_PROTECT,ADDSLASHES_FORCE). "' WHERE id = '" . $id_pics . "'", __LINE__, __FILE__);
