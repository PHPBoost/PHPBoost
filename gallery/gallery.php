...
$g_sort = AppContext::get_request()->get_getstring('sort', '');
...
elseif (isset($_FILES['gallery'])) //Upload
{
	...
	$idpic = 0;
	$idcat_post = AppContext::get_request()->get_poststring('cat', '');
	$name_post = AppContext::get_request()->get_poststring('name', '');
...
else
{
	...
	if ($nbr_pics > 0)
	{
		...
		if (!empty($g_idpics))
		{
			...
			if (!empty($info_pics['id']))
			{
				...
				$Template->put_all(array(
					...
					'COM' => '<a href="'. PATH_TO_ROOT .'/gallery/gallery' . url('.php?cat=' . $info_pics['idcat'] . '&amp;id=' . $info_pics['id'] . '&amp;com=0&amp;sort=' . $g_sort, '-' . $info_pics['idcat'] . '-' . $info_pics['id'] . '.php?com=0&amp;sort=' . $g_sort) .'">'. CommentsService::get_number_and_lang_comments($comments) . '</a>',
		...
		else
		{
			...
			$sort = AppContext::get_request()->get_getstring('sort', '');
			$Template->put_all(array(
