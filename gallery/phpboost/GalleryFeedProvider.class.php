...
class GalleryFeedProvider implements FeedProvider
{
	...
	{
		return CategoriesService::get_categories_manager('gallery', 'idcat')->get_feeds_categories_module()->get_feed_list();
	}

	function get_feed_data_struct($idcat = 0, $name = '')
	{
		if (CategoriesService::get_categories_manager('gallery', 'idcat')->get_categories_cache()->category_exists($idcat))
		{
			$category = CategoriesService::get_categories_manager('gallery', 'idcat')->get_categories_cache()->get_category($idcat);
			...
			$categories = CategoriesService::get_categories_manager('gallery', 'idcat')->get_children($idcat, new SearchCategoryChildrensOptions(), true);
			...
			foreach ($results as $row)
			{
				...
				$item->set_auth(CategoriesService::get_categories_manager('gallery', 'idcat')->get_heritated_authorizations($row['idcat'], Category::READ_AUTHORIZATIONS, Authorizations::AUTH_PARENT_PRIORITY));
