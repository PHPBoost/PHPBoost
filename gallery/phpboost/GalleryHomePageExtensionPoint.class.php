		...
		if ($total_cat > 0 && $nbr_unauth_cats < $total_cat && empty($g_idpics))
		{
			...
			while ($row = $this->sql_querier->fetch_assoc($result))
			{
				...
				$Template->assign_block_vars('cat_list', array(
					...
					'EDIT' => $is_admin ? '<a href="'. PATH_TO_ROOT.'/gallery/admin_gallery_cat.php?id=' . $row['id'] . '" title="' . $LANG['cat_edit'] . '" class="pbt-icon-edit"></a>' : '',
			...
			if (!empty($g_idpics))
			{
				...
				if (!empty($info_pics['id']))
				{
					...
					$Template->put_all(array(
						...
						'POSTOR' => '<a class="small_link ' . UserService::get_level_class($info_pics['level']) . '"' . (!empty($group_color) ? ' style="color:' . $group_color . '"' : '') . ' href="'. UserUrlBuilder::profile($info_pics['user_id'])->rel() .'">' . $info_pics['login'] . '</a>',
			...
			else
			{
				...
				while ($row = $this->sql_querier->fetch_assoc($result))
				{
					...
					$Template->assign_block_vars('pics_list', array(
						...
						'POSTOR' => $config->is_author_displayed() ? '<br />' . $LANG['by'] . (!empty($row['login']) ? ' <a class="small_link '.UserService::get_level_class($row['level']).'"' . (!empty($group_color) ? ' style="color:' . $group_color . '"' : '') . ' href="'. UserUrlBuilder::profile($row['user_id'])->rel() .'">' . $row['login'] . '</a>' : ' ' . $LANG['guest']) : '',
