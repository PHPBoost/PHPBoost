...
else
{
	...
	$total_cat = $sql->query("SELECT COUNT(*) FROM ".PREFIX."gallery_cats AS gc " . $clause_cat, __LINE__, __FILE__);
	...
	$template->assign_vars(array(
		...
		'PAGINATION' => $pagination->show_pagin('admin_gallery.php?p=%d', $total_cat, 'p', $CONFIG_GALLERY['nbr_pics_max'], 3),	
		...
		'ADD_PICS' => '<a href="admin_gallery_add.php?cat=' . $idcat . '"><img src="../templates/' . $CONFIG['theme'] . '/images/' . $CONFIG['lang'] . '/add.png" alt="" class="valign_middle" /></a>',
		'ARRAY_JS' => '',
		'NBR_PICS' => 0,
		'MAX_START' => 0,
		'START_THUMB' => 0,
	...
	if( $total_cat > 0 )
	{
		...
		FROM ".PREFIX."gallery_cats AS gc
		LEFT JOIN ".PREFIX."gallery AS g ON g.idcat = gc.id
		" . $clause_cat . "
		GROUP BY gc.id
		ORDER BY gc.id_left
	...
	if( $nbr_pics > 0 )
	{
		$template->assign_block_vars('pics', array(
			'EDIT' => '<a href="admin_gallery_cat.php' . (!empty($idcat) ? '?id=' . $idcat : '') . '"><img class="valign_middle" src="../templates/' . $CONFIG['theme'] . '/images/' . $CONFIG['lang'] . '/edit.png" alt="" /></a>',
		...
		$template->assign_vars(array(
			'PAGINATION_PICS' => $pagination->show_pagin('admin_gallery.php?cat=' . $idcat . '&amp;pp=%d', $nbr_pics, 'pp', $CONFIG_GALLERY['nbr_pics_max'], 3),
		));
		...
		if( !empty($idpics) )
		{
			...
			LEFT JOIN ".PREFIX."member AS m ON m.user_id = g.user_id		
			...
			if( !empty($info_pics['id']) )
			{
				...
				$template->assign_block_vars('pics.pics_max', array(
					...
					'DATE' => date($LANG['date_format'], $info_pics['timestamp']),
					'VIEWS' => ($info_pics['views'] + 1),
					'DIMENSION' => $info_pics['width'] . ' x ' . $info_pics['height'],
					'SIZE' => arrondi($info_pics['weight']/1024, 1),
					'COLSPAN' => ($CONFIG_GALLERY['nbr_column'] + 2),	
					'CAT' => $cat_list,	
					'RENAME' => addslashes($info_pics['name']),
					'RENAME_CUT' => addslashes($info_pics['name']),		
					'IMG_APROB' => ($info_pics['aprob'] == 1) ? 'unaprob.png' : 'aprob.png',
					'U_DEL' => 'php?del=' . $info_pics['id'] . '&amp;cat=' . $idcat,
					'U_MOVE' => '.php?id=' . $info_pics['id'] . '&amp;move=\' + this.options[this.selectedIndex].value',
					'U_PREVIOUS' => ($pos_pics > 0) ? '<a href="admin_gallery.php?cat=' . $idcat . '&amp;id=' . $id_previous . '#pics_max"><img src="../templates/' . $CONFIG['theme'] . '/images/left.png" alt="" class="valign_middle" /></a> <a href="admin_gallery.php?cat=' . $idcat . '&amp;id=' . $id_previous . '#pics_max">' . $LANG['previous'] . '</a>' : '',
					'U_NEXT' => ($pos_pics < ($i - 1)) ? '<a href="admin_gallery.php?cat=' . $idcat . '&amp;id=' . $id_next . '#pics_max">' . $LANG['next'] . '</a> <a href="admin_gallery.php?cat=' . $idcat . '&amp;id=' . $id_next . '#pics_max"><img src="../templates/' . $CONFIG['theme'] . '/images/right.png" alt="" class="valign_middle" /></a>' : '',
		...
		else
		{
			...
			while( $row = $sql->sql_fetch_assoc($result) )
			{
				...
				$template->assign_block_vars('pics.list', array(
					...
					'PATH' => $row['path'],
					'NAME' => $name_cut,
					'RENAME_FILE' => '<span id="fihref' . $row['id'] . '"><a href="javascript:display_rename_file(\'' . $row['id'] . '\', \'' . addslashes($row['name']) . '\', \'' . addslashes($name_cut) . '\');"><img src="../templates/' . $CONFIG['theme'] . '/images/' . $CONFIG['lang'] . '/edit.png" alt="' . $LANG['edit'] . '" class="valign_middle" /></a></span>',
					'IMG_APROB' => ($row['aprob'] == 1) ? 'unaprob.png' : 'aprob.png',
					'TR_START' => $tr_start,
					'TR_END' => $tr_end,
					'CAT' => $cat_list,
