...
include_once('../articles/articles_auth.php');
...
{ 
	...
	$cat_links = array($LANG['title_articles'] => transid('articles.php'));
	foreach($CAT_ARTICLES as $id => $array_info_cat)
	{
		if( $CAT_ARTICLES[$idartcat]['id_left'] >= $array_info_cat['id_left'] && $CAT_ARTICLES[$idartcat]['id_right'] <= $array_info_cat['id_right'] && $array_info_cat['level'] <= $CAT_ARTICLES[$idartcat]['level'] )
			$cat_links[$array_info_cat['name']] = 'articles' . transid('.php?cat=' . $id, '-' . $id . '.php');
	}
	if( !empty($idart) )
	{
		...
		$cat_links[$articles['title']] = 'articles' . transid('.php?cat=' . $idartcat . '&amp;id=' . $idart, '-' . $idartcat . '-' . $idart . '+' . url_encode_rewrite($articles['title']) . '.php');	
...
{
	$cat_links = array($LANG['title_articles'] => '');
...
if( !empty($idart) && isset($_GET['cat']) )
{
	...
	$template->assign_block_vars('article', array(
		...
		'PAGINATION' => $pagination->show_pagin('articles' . transid('.php?cat=' . $idartcat . '&amp;id='. $idart . '&amp;p=%d', '-' . $idartcat . '-'. $idart . '-%d+' . url_encode_rewrite($articles['title']) . '.php'), $nbr_page, 'p', 1, 3),
		'COM' => $link . $com,
		'NOTE' => $note,
...
else
{
	...
	$total_cat = $sql->query("SELECT COUNT(*) FROM ".PREFIX."articles_cats AS ac " . $clause_cat, __LINE__, __FILE__);	
	...
	if( $total_cat > 0 && count($unauth_cats_sql) < $total_cat )
	{
		...
		$i = 0;	
		$result = $sql->query_while("SELECT ac.id, ac.name, ac.icon, (ac.nbr_articles_visible + ac.nbr_articles_unvisible) AS nbr_articles, ac.nbr_articles_unvisible
		FROM ".PREFIX."articles_cats AS ac
