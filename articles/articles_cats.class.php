...
class ArticlesCats extends CategoriesManager
{
	...
	function add($id_parent, $name, $description, $image, $auth,$tpl_articles,$tpl_cat,$options)
	{
		global $Sql;
		if (array_key_exists($id_parent, $this->cache_var) || $id_parent == 0)
		{
			$new_id_cat = parent::add($id_parent, $name);
			$Sql->query_inject("UPDATE " . DB_TABLE_ARTICLES_CAT . " SET description = '" . $description . "', image = '" . $image . "', auth = '" . $auth . "',tpl_articles='".$tpl_articles."',tpl_cat='".$tpl_cat."',options='".$options."' WHERE id = '" . $new_id_cat . "'", __LINE__, __FILE__);
	...
	function update_category($id_cat, $id_parent, $name, $description, $image, $auth,$tpl_articles,$tpl_cat,$options)
	{
		global $Sql, $Cache;
		if (array_key_exists($id_cat, $this->cache_var))
		{
			...
			$Sql->query_inject("UPDATE " . DB_TABLE_ARTICLES_CAT . " SET name = '" . $name . "', image = '" . $image . "', description = '" . $description . "', auth = '" . $auth . "',tpl_articles='".$tpl_articles."',tpl_cat='".$tpl_cat."',options='".$options."' WHERE id = '" . $id_cat . "'", __LINE__, __FILE__);
