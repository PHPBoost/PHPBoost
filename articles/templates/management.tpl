...
<form action="management.php?token={TOKEN}" name="form" method="post"
	onsubmit="return check_form();" class="fieldset_content" id="form">
	<fieldset>
		...
		<dl>
			...
			<dd>
				<label><input type="text" size="65" maxlength="100" id="title"
					name="title" value="{TITLE_ART}" class="text" /> </label>
			</dd>
		</dl>
		<dl>
			<dt>
			...
			<dd>
				<label> <select id="{FORM_ID}" name="{FORM_NAME}">
						<option value="0">{L_ROOT}</option> # START options #
						<option value="{options.ID}"{options.SELECTED_OPTION}>{options.PREFIX}
							{options.NAME}</option> # END options #
				</select> </label>
			</dd>
		</dl>
		...
		<dl>
			<dt>
				<label for="special_auth">{L_MODELS}</label>
			
			
			<dd>
				<select id="models" name="models"
					onchange="ajax_model_extend_field()"> {MODELS}
				</select>
			</dd>
		</dl>
			...
			<dd>
				<label> <select name="icon"
					...
					onclick="change_icon(this.options[this.selectedIndex].value)">
						{IMG_LIST}
				</select> <span id="icon_img">{IMG_ICON}</span> <br /> <span
					class="text_small">{L_OR_DIRECT_PATH}</span> <input size="40"
					type="text" class="text" name="icon_path" value="{IMG_PATH}"
					onblur="if( this.value != '' )change_icon(this.value)" /> </label>
			</dd>
		</dl>
		<div id="preview"></div>
		<label for="description" id="scroll_contents">{L_ARTICLE_DESCRIPTION}</label>
		{KERNEL_EDITOR_DESC} <label><textarea rows="4" cols="86"
				id="description" name="description">{DESCRIPTION}</textarea> </label>
		<br /> <label for="contents" id="scroll_contents">* {L_TEXT}</label>
		{KERNEL_EDITOR} <label><textarea rows="20" cols="86" id="contents"
				name="contents">{CONTENTS}</textarea> </label>
		...
		<table
			...
			<tr>
				<th style="text-align: center;">{L_SOURCE}</th>
				<th style="text-align: center;">{L_SOURCE_LINK}</th>
			</tr>
			<tr>
				<td class="row2" style="text-align: center;"># START sources # <label><input
						type="text" size="40" name="a{sources.I}" id="a{sources.I}"
						value="{sources.SOURCE}" class="text" /> </label><br /> # END
					sources # <span id="a{NB_SOURCE}"></span>
				</td>
				<td class="row2" style="text-align: center;"># START sources # <label><input
						type="text" size="40" name="v{sources.I}" id="v{sources.I}"
						value="{sources.URL}" class="text" /> </label><br /> # END sources
			...
			<tr>
				<td style="text-align: center;" colspan="2">
					<div id="s{NB_SOURCE}"
						...
						<a href="javascript:add_field({NB_SOURCE}, 100)"><img
							style="vertical-align: bottom;"
							src="../templates/{THEME}/images/form/plus.png" alt="+" />&nbsp;&nbsp;{L_ADD_SOURCE}</a>
					</div>
				</td>
			</tr>
		</table>
		<br /> <br /> # IF NOT C_CONTRIBUTION #
		<fieldset>
			<legend> {L_SPECIAL_AUTH} </legend>
			<dl>
				<dt>
					<label for="special_auth">{L_SPECIAL_AUTH}</label> <br /> <span
						class="text_small">{L_SPECIAL_AUTH_EXPLAIN_ARTICLES}</span>
				</dt>
				<dd>
		...
		<dl class="overflow_visible">
			...
			<dd>
				<div
					...
					<input type="radio" value="2" name="visible" id="start_end_date"
						# IF VISIBLE_WAITING # checked="checked" # ENDIF # />
					{L_FROM_DATE} {START_CALENDAR} {L_AT} <input type="text" size="2"
						maxlength="2" name="start_hour" value="{START_HOUR}" class="text" />
					&nbsp;{L_UNIT_HOUR}&nbsp; <input type="text" size="2" maxlength="2"
						name="start_min" value="{START_MIN}" class="text" />
					&nbsp;{L_TO_DATE}&nbsp; {END_CALENDAR} {L_AT} <input type="text"
						size="2" maxlength="2" name="end_hour" value="{END_HOUR}"
						class="text" /> &nbsp;{L_UNIT_HOUR}&nbsp; <input type="text"
						size="2" maxlength="2" name="end_min" value="{END_MIN}"
						class="text" />
				</div>
				<input type="radio" value="1" name="visible" # IF
					VISIBLE_ENABLED # checked="checked" # ENDIF # id="release_date" />
				{L_IMMEDIATE} <br /> <input type="radio" value="0" name="visible"
					# IF VISIBLE_UNAPROB # checked="checked" # ENDIF # /> {L_UNAPROB}
			</dd>
		</dl>
		# ENDIF #
		<dl class="overflow_visible">
			...
			<dd>
				{RELEASE_CALENDAR} {L_AT} <input type="text" size="2" maxlength="2"
					id="release_hour" name="release_hour" value="{RELEASE_HOUR}"
					class="text" /> &nbsp;{L_UNIT_HOUR}&nbsp; <input type="text"
					size="2" maxlength="2" id="release_min" name="release_min"
					value="{RELEASE_MIN}" class="text" /> # IF C_ADD # &nbsp; <input
					type="button" id="button_new" value="{L_IMMEDIATE}" class="submit"
					onclick="javascript:value_now('{RELEASE_CALENDAR_ID}', 'release_hour', 'release_min')" />
				# ENDIF #
			</dd>
		</dl>
	</fieldset>
	...
	<fieldset>
		...
		<p>
			<label for="counterpart">{L_CONTRIBUTION_COUNTERPART}</label>
		</p>
	...
	<fieldset class="fieldset_submit">
		...
		<input type="hidden" id="id" name="id" value="{IDARTICLES}"
			class="submit" /> <input type="hidden" id="user_id" name="user_id"
			value="{USER_ID}" class="submit" /> <input type="submit"
