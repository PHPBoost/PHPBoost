...
class ArticlesInterface extends ModuleInterface
{
	...
	function get_home_page()
	{
		...
		$tpl->assign_vars(array(
			'C_WAITING'=> false,
			'U_ARTICLES_WAITING'=>' <a href="articles.php?user=1">' . $ARTICLES_LANG['waiting_articles'] . '</a>&nbsp;<a href="' . TPL_PATH_TO_ROOT . '/member/member' . url('.php?id=' . $User->get_attribute('user_id'), '-' . $User->get_attribute('user_id') . '.php') . '" class="' . $array_class[$User->get_attribute('level')] . '"' . (!empty($group_color) ? ' style="color:' . $group_color . '"' : '') . '>' . wordwrap_html($User->get_attribute('login'), 19) . '</a><br />',
			'IDCAT' => $idartcat,
			'C_IS_ADMIN' => $is_admin,
