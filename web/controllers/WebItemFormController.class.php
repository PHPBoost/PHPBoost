...
class WebItemFormController extends ModuleController
{
	...
	private function build_form(HTTPRequestCustom $request)
	{
		...
		$options_fieldset->add_field(new FormFieldThumbnail('thumbnail', $this->common_lang['form.picture'], $this->get_weblink()->get_thumbnail()->relative(), WebItem::THUMBNAIL_URL));
		...
		if (CategoriesAuthorizationsService::check_authorizations($this->get_weblink()->get_id_category())->moderation())
		{
			...
			if (!$this->get_weblink()->is_published())
			{
				$publication_fieldset->add_field(new FormFieldCheckbox('update_creation_date', $this->common_lang['form.update.date.creation'], false,
					array('hidden' => $this->get_weblink()->get_status() != WebItem::NOT_APPROVAL)
				));
			}
			...
			$publication_fieldset->add_field($publishing_start_date = new FormFieldDateTime('publishing_start_date', $this->common_lang['form.date.start'], ($this->get_weblink()->get_publishing_start_date() === null ? new Date() : $this->get_weblink()->get_publishing_start_date()),
				array('hidden' => ($request->is_post_method() ? ($request->get_postint(__CLASS__ . '_published', 0) != WebItem::APPROVAL_DATE) : ($this->get_weblink()->get_publishing_state() != WebItem::APPROVAL_DATE)))
			));
			...
			$publication_fieldset->add_field(new FormFieldCheckbox('end_date_enabled', $this->common_lang['form.date.end.enable'], $this->get_weblink()->is_end_date_enabled(),
				array(
					'hidden' => ($request->is_post_method() ? ($request->get_postint(__CLASS__ . '_published', 0) != WebItem::APPROVAL_DATE) : ($this->get_weblink()->get_publishing_state() != WebItem::APPROVAL_DATE)),
					'events' => array('click' => '
						if (HTMLForms.getField("end_date_enabled").getValue()) {
							HTMLForms.getField("publishing_end_date").enable();
						} else {
	...
	private function save()
	{
		...
		if (!CategoriesAuthorizationsService::check_authorizations($item->get_id_category())->moderation())
		{
			...
			if (CategoriesAuthorizationsService::check_authorizations($item->get_id_category())->contribution() && !CategoriesAuthorizationsService::check_authorizations($item->get_id_category())->write())
				$item->set_publishing_state(WebItem::NOT_APPROVAL);
		}
		else
		{
			...
			if ($item->get_publishing_state() == WebItem::APPROVAL_DATE)
			{
