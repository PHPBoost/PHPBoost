...
function show_cat_contents($id_cat, $cats, $id, $display_select_link)
{
	...
	foreach ($_PAGES_CATS as $key => $value)
	{
		...
		if ($value['id_parent']  == $id_cat)
		{
			...
			{
				$line .= '<li class="sub"><a class="parent" href="javascript:show_cat_contents(' . $key . ', ' . ($display_select_link != 0 ? 1 : 0) . ');"><span class="icon-minus-square-o" id="img2_' . $key . '"></span><span class="icon-folder-open" id="img_' . $key . '"></span></a><a id="class_' . $key . '" class="' . ($key == $id ? 'selected' : '') . '" href="javascript:' . ($display_select_link != 0 ? 'select_cat' : 'open_cat') . '(' . $key . ');">' . $value['name'] . '</a><span id="cat_' . $key . '">
				<ul>'
			...
			else
			{
				...
				if ($sub_cats_number > 0)
					$line .= '<li class="sub"><a class="parent" href="javascript:show_cat_contents(' . $key . ', ' . ($display_select_link != 0 ? 1 : 0) . ');"><span class="icon-plus-square-o" id="img2_' . $key . '"></span><span class="icon-folder" id="img_' . $key . '"></span></a><a class="' . ($key == $id ? 'selected' : '') . '" id="class_' . $key . '" href="javascript:' . ($display_select_link != 0 ? 'select_cat' : 'open_cat') . '(' . $key . ');">' . $value['name'] . '</a><span id="cat_' . $key . '"></span></li>';
				else //Sinon on n'affiche pas le "+"
					$line .= '<li class="sub"><a id="class_' . $key . '" class="' . ($key == $id ? 'selected' : '') . '" href="javascript:' . ($display_select_link != 0 ? 'select_cat' : 'open_cat') . '(' . $key . ');"><span class="icon-folder"></span>' . $value['name'] . '</a></li>';
			}
		}
	}
	return "\n" . $line;
