...
class PagesHomePageExtensionPoint implements HomePageExtensionPoint
{
	...
	private function get_view()
	{
		...
		while ($row = $this->sql_querier->fetch_assoc($result))
		{
			...
			{
				$root .= '<li><a href="' . PagesUrlBuilder::get_link_item($row['encoded_title']) . '"><span class="icon-file"></span>' . $row['title'] . '</a></li>';
			}
		}
		...
		while ($row = $this->sql_querier->fetch_assoc($result))
		{
			...
			{	
				$tpl->assign_block_vars('list', array(
					'DIRECTORY' => '<li class="sub"><a class="parent" href="javascript:show_cat_contents(' . $row['id'] . ', 0);"><img src="' . $tpl->get_pictures_data_path() . '/images/plus.png" alt="" id="img2_' . $row['id'] . '"/><span class="icon-folder-close" id ="img_' . $row['id'] . '"></span></a><a id="class_' . $row['id'] . '" href="javascript:open_cat(' . $row['id'] . ');">' . $row['title'] . '</a><span id="cat_' . $row['id'] . '"></li>'
				));
			}
			else
			{
