	...
	while ($row = $result->fetch())
	{
		...
		if (($special_auth && AppContext::get_current_user()->check_auth($row['auth'], READ_PAGE)) || (!$special_auth && AppContext::get_current_user()->check_auth($config_authorizations, READ_PAGE)))
		{
			...
			if ($sub_cats_number > 0)
				echo '<li class="sub"><a class="parent" href="javascript:show_pages_cat_contents(' . $row['id'] . ', ' . ($display_select_link != 0 ? 1 : 0) . ');"><i class="far fa-plus-square" id="img-subfolder-' . $row['id'] . '"></i><i class="fas fa-folder" id="img-folder-' . $row['id'] . '"></i></a><a id="class-' . $row['id'] . '" href="javascript:' . ($display_select_link != 0 ? 'select_cat' : 'open_cat') . '(' . $row['id'] . ');">' . stripslashes($row['title']) . '</a><span id="cat-' . $row['id'] . '"></span></li>';
			else //Sinon on n'affiche pas le "+"
				echo '<li class="sub"><a id="class-' . $row['id'] . '" href="javascript:' . ($display_select_link != 0 ? 'select_cat' : 'open_cat') . '(' . $row['id'] . ');"><i class="fas fa-folder"></i>' . stripslashes($row['title']) . '</a></li>';
		}
	}
	$result->dispose();
	echo '</ul>';
...
elseif (!empty($open_cat) || $root == 1)
{
	...
	foreach ($categories as $key => $cat)
	{
		if ($cat['id_parent'] == $open_cat)
		{
			...
			{
				$return .= '<li><a href="javascript:open_cat(' . $key . '); show_pages_cat_contents(' . $cat['id_parent'] . ', 0);"><i class="fas fa-folder"></i>' . stripslashes($cat['title']) . '</a></li>';
			}
		}
	}
	...
	while ($row = $result->fetch())
	{
		...
		{
			$return .= '<li><a href="' . PATH_TO_ROOT . url('/pages/pages.php?title=' . $row['encoded_title'], '/pages/' . $row['encoded_title']) . '"><i class="fas fa-file"></i>' . stripslashes($row['title']) . '</a></li>';
		}
	}
	$result->dispose();
	$return .= '</ul>';
