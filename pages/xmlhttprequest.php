...
if( $id_cat != 0 )
{
	...
	FROM ".PREFIX."pages_cats AS c
	LEFT JOIN ".PREFIX."pages AS p ON p.id = c.id_page
	...
	while( $row = $sql->sql_fetch_assoc($result) )
	{
		...
		if( ($special_auth && $groups->check_auth($row['auth'], READ_PAGE)) || (!$special_auth && $groups->check_auth($_PAGES_CONFIG['auth'], READ_PAGE)) )
		{
...
elseif( !empty($open_cat) || $root == 1 )
{
	...
	while( $row = $sql->sql_fetch_assoc($result) )
	{
		...
		$special_auth = !empty($row['auth']);
		//Vérification de l'autorisation d'éditer la page
		if( ($special_auth && $groups->check_auth(unserialize($row['auth']), READ_PAGE)) || (!$special_auth && $groups->check_auth($_PAGES_CONFIG['auth'], READ_PAGE)) )
		{
