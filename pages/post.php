...
$del_page = !empty($_GET['del']) ? numeric($_GET['del']) : 0;
...
if( $id_edit > 0 )
{
	...
	$speed_bar = array();
	$speed_bar[TITLE] = transid('post.php?id=' . $id_edit);
	...
	$speed_bar = array($LANG['pages'] => transid('pages.php'), TITLE => '');
...
elseif( $del_page > 0 )
{
	$page_infos = $sql->query_array('pages', 'id', 'title', 'encoded_title', 'contents', 'auth', 'count_hits', 'activ_com', 'id_cat', 'is_cat', "WHERE id = '" . $del_page . "'", __LINE__, __FILE__);
	...
	{
		$sql->query_inject("DELETE FROM ".PREFIX."pages WHERE id = '" . $del_page . "'", __LINE__, __FILE__);
		$sql->query_inject("DELETE FROM ".PREFIX."pages WHERE redirect = '" . $del_page . "'", __LINE__, __FILE__);
		$sql->query_inject("DELETE FROM ".PREFIX."com WHERE script = 'pages' AND idprov = '" . $del_page . "'", __LINE__, __FILE__);
