...
class PagesCategoriesCache extends DefaultRichCategoriesCache
{
	...
	{
		$now = new Date();
		return PagesService::count('WHERE id_category = :id_category AND (published = 1 OR (published = 2 AND publishing_start_date < :timestamp_now AND (publishing_end_date > :timestamp_now OR publishing_end_date = 0)))',
			array(
