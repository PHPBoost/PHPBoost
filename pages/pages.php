...
if( !empty($encoded_title) ) //Si on connait son titre
{
	...
	$speed_bar = array();
	$speed_bar[$page_infos['title']] = transid('pages.php?title=' . $encoded_title, $encoded_title);
...
elseif( $id_com > 0 )
{
	...
	$speed_bar = array();
	$speed_bar[$LANG['pages_com']] = transid('pages.php?com=' . $id_com);
...
else
{
	...
	if( $groups->check_auth($_PAGES_CONFIG['auth'], READ_PAGE) )
		$speed_bar = array($LANG['pages'] => transid('pages.php'));
}
