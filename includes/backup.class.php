...
class backup extends Sql
{
	...
	function list_table()
	{
		if( $this->tables === array() )
			$this->tables = parent::sql_list_tables();
	}
	...
	function create_tables($table_list = array())
	{
		...
		foreach($this->tables as $id => $properties)
		{
			if( in_array($properties['name'], $table_list) || $all_tables )
			{
				$result = parent::query_while('SHOW CREATE TABLE ' . $properties['name'], __LINE__, __FILE__);
				while($row = parent::sql_fetch_row($result))
					$this->save .=  $row[1] . ';' . "\n\n";
				parent::close($result);
			}				
		}
	...
	function insert_values($tables = array())
	{
		...
		foreach($this->tables as $id => $table_info)
		{
			...
			{
				$rows_number = parent::query("SELECT COUNT(*) FROM " . $table_info['name'], __LINE__, __FILE__);
				if( $rows_number > 0 )
				{
					$this->save .= "INSERT INTO " . $table_info['name'] . " (`";
					$this->save .= implode('`, `', parent::sql_list_fields($table_info['name']));
					$this->save .= "`) VALUES ";
					
					$i = 1;
					$list_fields = parent::sql_list_fields($table_info['name']);
					$result = parent::query_while('SELECT * FROM ' . $table_info['name'], __LINE__, __FILE__);			
					while($row = parent::sql_fetch_row($result))
					{
	...
	{		
		if( count($table_array) != 0 )
			parent::query_inject("OPTIMIZE TABLE " . implode(', ', $table_array), __LINE__, __FILE__);
	}
	...
	{
		if( count($table_array) != 0 )
			parent::query_inject("REPAIR TABLE " . implode(', ', $table_array), __LINE__, __FILE__);
	}
	...
	function sql_highlight_string($query)
	{
		...
		$key_words = array('select', 'update', 'delete', 'insert into', 'truncate', 'alter', 'table', 'set', 'drop', 'from', 'values', 'count', 'distinct', 'having', 'left', 'right', 'join', 'natural', 'outer', 'inner', 'between', 'where', 'group by', 'order by', 'limit', 'or', 'and', 'not', 'in', 'as', 'on', 'all', 'any', 'like', 'concat', 'substring', 'collate', 'collation', 'primary', 'key', 'default', 'null', 'exists');
