...
class Sessions extends Sql
{
	...
	function session_begin($user_id, $password, $level, $session_script, $session_script_get, $session_script_title, $autoconnect = false)
	{
		...
		$check_ip = parent::query("SELECT COUNT(*) FROM ".PREFIX."compteur WHERE ip = '" . USER_IP . "'", __LINE__, __FILE__);
		if( empty($check_ip) && $this->check_robot(USER_IP) === false )
		{
			//Récupération forcée de la valeur du total de visites, car problème de CAST avec postgresql.
			parent::query_inject("UPDATE ".LOW_PRIORITY." ".PREFIX."compteur SET ip = ip + 1, time = " . parent::sql_now() . ", total = total + 1 WHERE id = 1", __LINE__, __FILE__);
			parent::query_inject("INSERT ".LOW_PRIORITY." INTO ".PREFIX."compteur (ip, time, total) VALUES('" . USER_IP . "', " . parent::sql_now() . ", 0)", __LINE__, __FILE__);
			
			//On lance les stats.
			include_once('../includes/save_stats.php');

			//Mise à jour du last_connect, pour un membre qui vient d'arriver sur le site.
			if( $user_id !== '-1' ) parent::query_inject("UPDATE ".PREFIX."member SET last_connect = '" . time() . "' WHERE user_id = '" . $user_id . "'", __LINE__, __FILE__);
		}
		...
		if( $user_id !== '-1' )
		{
			...
			$password_m = parent::query("SELECT password FROM ".PREFIX."member WHERE user_id = '" . $user_id . "' AND user_warning < 100 AND '" . time() . "' - user_ban >= 0", __LINE__, __FILE__);
			
			if( !empty($password) && $password === $password_m ) //Succès!
			{
				parent::query_inject("INSERT INTO ".PREFIX."sessions VALUES('" . $session_uniq_id . "', '" . $user_id . "', '" . $level . "', '" . USER_IP . "', '" . time() . "', '" . $session_script . "', '" . $session_script_get . "', '" . $session_script_title . "', '')", __LINE__, __FILE__);				
				$cookie_on = true; //Génération du cookie!			
			}
			else //Session visiteur, echec!
			{
				parent::query_inject("INSERT INTO ".PREFIX."sessions VALUES('" . $session_uniq_id . "', -1, -1, '" . USER_IP . "', '" . time() . "', '" . $session_script . "', '" . $session_script_get . "', '" . $session_script_title . "', '0')", __LINE__, __FILE__);		
				$delay_ban = parent::query("SELECT user_ban FROM ".PREFIX."member WHERE user_id = '" . $user_id . "'", __LINE__, __FILE__);
				if( (time - $delay_ban) >= 0 )
					$error = 'echec';
				else
					$error = $delay_ban;		
	...
	function get_session_autoconnect($session_script, $session_script_get, $session_script_title)
	{
		...
		if( isset($_COOKIE[$CONFIG['site_cookie'].'_autoconnect']) )
		{
			...
			$level = parent::query("SELECT level FROM ".PREFIX."member WHERE user_id = '" . $session_autoconnect['user_id'] . "' AND password = '" . $session_autoconnect['pwd'] . "'", __LINE__, __FILE__);
			...
			if( !empty($session_autoconnect['user_id']) && !empty($session_autoconnect['pwd']) && isset($level) )
			{
				...
				{
					parent::query_inject("UPDATE ".PREFIX."member SET last_connect='" . time() . "', test_connect = test_connect + 1 WHERE user_id='" . $session_autoconnect['user_id'] . "'", __LINE__, __FILE__);
					
					$test_connect = parent::query("SELECT test_connect FROM ".PREFIX."member WHERE user_id = '" . $session_autoconnect['user_id'] . "'", __LINE__, __FILE__);
				...
				else //Succès on recharge la page.
				{
					...
					parent::query_inject("UPDATE ".PREFIX."member SET last_connect = '" . time() . "' WHERE user_id = '" . $session_autoconnect['user_id'] . "'", __LINE__, __FILE__);
	...
	function get_session_id()
	{
		...
		if( isset($_COOKIE[$CONFIG['site_cookie'].'_data']) )
		{
			...
			$this->data['session_id'] = isset($session_data['session_id']) ? securit($session_data['session_id']) : ''; //Validité du session id.
		...
		{
			$this->data['session_id'] = !empty($_GET['sid']) ? securit($_GET['sid']) : ''; //Validité du session id.
	...
	function session_info()
	{
		...
		if( $this->data['user_id'] !== -1 && !empty($this->data['user_id']) )
			$userdata = parent::query_array('member', 'user_id', 'login', 'level', 'user_groups', 'user_lang', 'user_theme', 'user_mail', 'user_pm', 'user_readonly', "WHERE user_id='" . $this->data['user_id'] . "'", __LINE__, __FILE__);		
	...
	function session_check($session_script, $session_script_get, $session_script_title)
	{
		global $CONFIG;
		...
		else //Visiteur
		{
			...
			if( parent::sql_affected_rows($ressource, "SELECT COUNT(*) FROM ".PREFIX."sessions WHERE user_id = -1 AND session_ip = '" . USER_IP . "'") == 0 ) //Aucune session lancée.
			{
				if( isset($_COOKIE[$CONFIG['site_cookie'].'_data']) )
				{
					setcookie($CONFIG['site_cookie'].'_data', '', time() - 31536000, '/'); //Destruction cookie.
	...
	function session_end()
	{
		...
		parent::query_inject("DELETE FROM ".PREFIX."sessions WHERE session_id = '" . $this->data['session_id'] . "'", __LINE__, __FILE__);
