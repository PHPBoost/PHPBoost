...
if( $DEFINED_PHPBOOST )
{
	...
	if( !empty($_GET['i']) )
	{
		if( !preg_match('`([0-9]+)([a-z]+)([0-9]*)`', $_GET['i'], $array_get) )
			$array_get = array('', '', '', '');
...
if( isset($_com_script) && isset($_com_idprov) && isset($_com_vars) && isset($_com_vars_e) )
{
	...
	if( isset($info_module['com']) )
	{
		...
		{
			$info_sql_module = $sql->query_array(securit($info_module['com']), "COUNT(*) as nbr_com", "lock_com", "WHERE id = '" . $_com_idprov . "' GROUP BY id", __LINE__, __FILE__);
			if( !empty($info_sql_module['nbr_com']) )
				$check_script = true;
		}
	}
	...
	else
	{
		...
		$forbidden_tags = implode(', ', $CONFIG_COM['forbidden_tags']);
		$template->assign_vars(array(
			'PAGINATION' => $pagination->show_pagin($_com_path . transid($_com_vars_simple, $_com_vars_r_simple) . '&amp;pc=%d#' . $_com_script, $info_sql_module['nbr_com'], 'pc', $CONFIG_COM['com_max'], 3),
			'LANG' => $CONFIG['lang'],
			'IDCOM' => '',
			'IDPROV' => $_com_idprov,
			'SCRIPT' => $_com_script,
		...
		LEFT JOIN ".PREFIX."member AS m ON m.user_id = c.user_id
		LEFT JOIN ".PREFIX."sessions AS s ON s.user_id = c.user_id AND s.session_time > '" . (time() - $CONFIG['site_session_invit']) . "'
