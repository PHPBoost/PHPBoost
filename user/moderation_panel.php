	...
	if (empty($id_get)) //On liste les membres qui ont déjà un avertissement
	{
		...
		while ($row = $Sql->fetch_assoc($result))
		{
			...
			$moderation_panel_template->assign_block_vars('member_list', array(
				...
				'U_ACTION_USER' => '<a href="'. UserUrlBuilder::moderation_panel('warning', $row['user_id'])->rel() .'" class="icon-warning"></a>',
				'U_PROFILE' => UserUrlBuilder::profile($row['user_id'])->rel(),
				'U_PM' => UserUrlBuilder::personnal_message($row['user_id'])->rel()
			));
	...
	if (empty($id_get)) //On liste les membres qui ont déjà un avertissement
	{
		...
		while ($row = $Sql->fetch_assoc($result))
		{
			...
			$moderation_panel_template->assign_block_vars('member_list', array(
				...
				'U_PROFILE' => UserUrlBuilder::profile($row['user_id'])->rel(),
				'U_ACTION_USER' => '<a href="'. UserUrlBuilder::moderation_panel('ban', $row['user_id'])->rel()  .'" class="icon-forbidden"></a>',
				'U_PM' => UserUrlBuilder::personnal_message($row['user_id'])->rel(),
			));
