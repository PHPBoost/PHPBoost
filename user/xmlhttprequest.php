...
if ($id_cat != 0)
{
	...
	while ($row = $result->fetch())
	{
		...
		if ($sub_cats_number > 0)
			...
			echo '<li class="upload-no-sub-cat"><i class="fas fa-folder"></i>&nbsp;<span id="class-' . $row['id'] . '" class=""><a href="javascript:' . ($display_select_link != 0 ? 'select_cat' : 'open_cat') . '(' . $row['id'] . ');">' . $row['name'] . '</a></span></li>';
	}
	$result->dispose();
	echo '</ul>';
}
