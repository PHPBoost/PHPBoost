...
function show_cat_contents($id_cat, $cats, $id, $display_select_link, $user_id)
{
	...
	while ($row = $Sql->fetch_assoc($result))
	{
		...
		{
			$line .= '<li><a href="javascript:show_cat_contents(' . $row['id'] . ', ' . ($display_select_link != 0 ? 1 : 0) . ');"><i class="icon-minus-square-o" id="img2_' . $row['id'] . '"></i></a> <a href="javascript:show_cat_contents(' . $row['id'] . ', ' . ($display_select_link != 0 ? 1 : 0) . ');"><i class="icon-folder-open" id="img_' . $row['id'] . '"></i></a>&nbsp;<span id="class_' . $row['id'] . '" class="' . ($row['id'] == $id ? 'upload-selected-cat' : '') . '"><a href="javascript:' . ($display_select_link != 0 ? 'select_cat' : 'open_cat') . '(' . $row['id'] . ');">' . $row['name'] . '</a></span><span id="cat_' . $row['id'] . '">
