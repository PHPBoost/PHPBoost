...
if ($convers && empty($pm_edit) && empty($pm_del)) //Envoi de conversation.
{
	...
	if (PrivateMsg::count_conversations(AppContext::get_current_user()->get_id()) >= $limit_group && (!AppContext::get_current_user()->check_level(User::MODERATOR_LEVEL) && !($limit_group === -1))) //Boîte de l'expéditeur pleine.
...
elseif (!empty($_POST['pm']) && !empty($pm_id_get) && empty($pm_edit) && empty($pm_del)) //Envoi de messages.
{
	...
	if (!empty($contents))
	{
		...
		if ($convers['user_convers_status'] == '0' && $user_id_dest > '0') //On vérifie que la conversation n'a pas été supprimée chez le destinataire, et que ce n'est pas un mp automatique du site.
		{
			...
			if ($convers['user_id'] == AppContext::get_current_user()->get_id()) //Le membre est le créateur de la conversation.
				$status = 1;
...
elseif (!empty($pm_id_get)) //Messages associés à la conversation.
{
	...
	$i = 0;
	$j = 0;
	$result = PersistenceContext::get_querier()->select("SELECT msg.id, msg.user_id, msg.timestamp, msg.view_status, m.display_name, m.level, m.email, m.show_email, m.timestamp AS registered, ext_field.user_avatar, m.posted_msg, m.user_warning, m.user_ban, m.groups, s.user_id AS connect, msg.contents
