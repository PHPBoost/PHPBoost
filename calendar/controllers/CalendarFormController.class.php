...
class CalendarFormController extends ModuleController
{
	...
	private function build_form(HTTPRequestCustom $request)
	{
		...
		$fieldset->add_field(new FormFieldNumberEditor('repeat_number', $this->lang['calendar.labels.repeat.number'], $item_content->get_repeat_number(),
			array('min' => 1, 'max' => 150, 'hidden' => ($request->is_post_method() ? ($request->get_poststring(__CLASS__ . '_repeat_type', '') == CalendarEventContent::NEVER) : $item_content->get_repeat_type() == CalendarEventContent::NEVER)),
	...
	private function get_event()
	{
		if ($this->item === null)
		{
			...
			else
			{
				...
				$item_content = new CalendarEventContent();
	...
	private function save()
	{
		...
		if ($item_content->get_repeat_type() != CalendarEventContent::NEVER)
	...
	private function set_event_start_and_end_date(CalendarEvent $item, $new_start_date, $new_end_date)
	{
	...
	private function contribution_actions(CalendarEvent $item, $id_event)
	{
