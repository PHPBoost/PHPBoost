...
class CalendarUnsuscribeController extends ModuleController
{
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		if (!empty($event_id))
		{
			...
			AppContext::get_response()->redirect(CalendarUrlBuilder::display_event($category->get_id(), $category->get_rewrited_name(), $event_id, $this->event->get_content()->get_rewrited_title()));
		}
		else
		{
	...
	private function check_authorizations()
	{
		...
		{
			$error_controller = new UserErrorController(LangLoader::get_message('error', 'errors-common'), LangLoader::get_message('calendar.notice.unsuscribe.event_date_expired', 'common', 'calendar'));
			DispatchManager::redirect($error_controller);
		}
