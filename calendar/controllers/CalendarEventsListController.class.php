...
class CalendarEventsListController extends ModuleController
{
	...
	private function build_table()
	{
		...
		foreach ($events as $event)
		{
			...
			$edit_link = new LinkHTMLElement(CalendarUrlBuilder::edit_event(!$event->get_parent_id() ? $event->get_id() : $event->get_parent_id()), '', array('title' => LangLoader::get_message('edit', 'common')), 'far fa-edit');
			$edit_link = $event->is_authorized_to_edit() ? $edit_link->display() : '';
			
			$delete_link = new LinkHTMLElement(CalendarUrlBuilder::delete_event($event->get_id()), '', array('title' => LangLoader::get_message('delete', 'common'), 'data-confirmation' => !$event->belongs_to_a_serie() ? 'delete-element' : ''), 'far fa-delete');
