...
include_once('../calendar/lang/' . $CONFIG['lang'] . '/calendar_' . $CONFIG['lang'] . '.php'); //Chargement de la langue du module.
...
if( $checkdate === true && empty($id) && empty($add) )
{
	...
	if( !empty($day) )
	{
		...
		LEFT JOIN ".PREFIX."member AS m ON m.user_id=cl.user_id
		...
		while( $row = $sql->sql_fetch_assoc($result) )
		{
			...
			$template->assign_block_vars('show.action', array(
				'DATE' => date($LANG['date_format'] . ' ' . $LANG['at'] . ' H:i', $row['timestamp']),
				'TITLE' => $row['title'],
...
elseif( !empty($id) )
{
	...
	elseif( !empty($edit) )
	{
		if( !empty($_POST['valid']) )
		{
			...
			$timestamp = strtotimestamp($date, $LANG['date_format']);
			if( $timestamp > 0 )
		...
		else //Formulaire d'édition
		{
			...
			$template->assign_block_vars('form', array(
				...
				'HOUR' => !empty($row['timestamp']) ? date('h', $row['timestamp']) : '',
				'MIN' => !empty($row['timestamp']) ? date('i', $row['timestamp']) : '',
