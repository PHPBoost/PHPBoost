<section id="module-calendar">
	...
	<div itemscope="itemscope" itemtype="http://schema.org/Event" id="article-calendar-{ID}" class="calendar-item single-item# IF C_NEW_CONTENT # new-content# ENDIF #">
		...
		<div class="flex-between">
			<div class="more">
					<i class="fa fa-user"></i> # IF C_AUTHOR_EXIST #<a itemprop="author" href="{U_AUTHOR_PROFILE}" class="{AUTHOR_LEVEL_CLASS}" # IF C_AUTHOR_GROUP_COLOR # style="color:{AUTHOR_GROUP_COLOR}" # ENDIF #>{AUTHOR}</a># ELSE #{AUTHOR}# ENDIF #
			</div>
			# IF C_CONTROLS #
				<div class="controls align-right">
					# IF C_EDIT #
						<a href="{U_EDIT}" aria-label="${LangLoader::get_message('edit', 'common')}"><i class="far fa-fw fa-edit" aria-hidden></i></a>
					# ENDIF #
					# IF C_DELETE #
						<a href="{U_DELETE}" aria-label="${LangLoader::get_message('delete', 'common')}"# IF NOT C_BELONGS_TO_A_SERIE # data-confirmation="delete-element"# ENDIF #><i class="far fa-fw fa-trash-alt" aria-hidden></i></a>
					# ENDIF #
				</div>
			# ENDIF #
		</div>
