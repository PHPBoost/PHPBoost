...
$speed_bar = array(
	$LANG['moderation_panel'] => transid('moderation_panel.php')
);
...
if( $action == 'punish' ) //Gestion des utilisateurs
{
	...
	$template->assign_block_vars('all_action', array(
	));
	...
	if( empty($id_get) ) //On liste les membres qui ont déjà un avertissement
	{
		...
		while( $row = $sql->sql_fetch_assoc($result) )
		{
			$template->assign_block_vars('user_list.list', array(
				...
				'INFO' => ($row['user_warning'] != 100) ? date($LANG['date_format'] . ' ' . $LANG['at'] . ' H\Hi', $row['user_ban']) : $LANG['illimited'],
				'U_PROFILE' => '../member/member' . transid('.php?id=' . $row['user_id'], '-' . $row['user_id'] . '.php'),
...
else //Accueil panneau de modération
{
