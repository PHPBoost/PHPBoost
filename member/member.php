...
$speed_bar = array(
	$LANG['member_area'] => transid('member.php?id=' . $session->data['user_id'] . '&amp;view=1', 'member-' . $session->data['user_id'] . '.php?view=1'), $title_mbr => ''
);
...
{	
	...
	{
		//Update profil
		$row = $sql->query_array('member', 'user_lang', 'user_theme', 'user_mail', 'user_local', 'user_web', 'user_occupation', 'user_hobbies', 'user_avatar', 'user_show_mail', 'user_sex', 'user_born', 'user_sign', 'user_desc', 'user_msn', 'user_yahoo', "WHERE user_id = '" . $session->data['user_id'] . "'", __LINE__, __FILE__);
		...
		if( $extend_field_exist > 0 )
		{
			...
			LEFT JOIN ".PREFIX."member_extend AS ex ON ex.user_id = '" . $id_get . "'
			WHERE exc.display = 1
	...
	elseif( !empty($_POST['valid']) && ($session->data['user_id'] === $id_get) && ($session->check_auth($session->data, 0)) ) //Update du profil
	{
		...
		if( preg_match("!^[a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4}$!", $mail) )
		{
			...
			if( !empty($user_mail) )
			{
				...
				" . $user_mail . "user_show_mail = '" . $user_show_mail . "', user_local = '" . $user_local . "', 
				...
				user_desc = '" . parse($user_desc) . "', user_sex = '" . $user_sex . "', user_born = '" . $user_born . "', 
				user_sign = '" . parse($user_sign) . "' WHERE user_id = '" . numeric($session->data['user_id']) . "'", __LINE__, __FILE__); 
		...
		if( $_POST['del_member'] === 'on' ) //Suppression du compte
		{
			...
			$stats_install = $sql->query("SELECT COUNT(*) FROM ".PREFIX."modules WHERE name = 'stats'", __LINE__, __FILE__);
			if( $stats_install == 1 )
				$cache->generate_module_file('stats');
		}
	}	
	...
	else  //Profil public du membre.
	{
		...
		$template->assign_block_vars('profil', array(
			...
			'LAST_CONNECT' => date($LANG['date_format'], $row['last_connect']),
...
else //Show all member!
{
	...
	$template->assign_vars(array(
		'PAGINATION' => '&nbsp;<strong>' . $LANG['page'] . ' :</strong> ' . $pagination->show_pagin('member' . transid('.php?' . $unget . '&amp;p=%d', '-0-%d.php' . $unget), $nbr_member, 'p', 25, 3)
	));
