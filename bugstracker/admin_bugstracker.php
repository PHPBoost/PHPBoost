...
if ( !empty($valid_insert_post) )
{
	...
	if( !empty($weight) AND !empty($label) AND !empty($nature) )
	{
		$Sql->query_inject("INSERT INTO ".PREFIX."bugstracker_parameters
			...
			__LINE__, __FILE__);
		$last_id = $Sql->sql_insert_id("SELECT MAX(id) FROM ".PREFIX."bugstracker_parameters");
...
{	
  	$Template->set_filenames(array(
		'admin_bugstracker' => 'bugstracker/admin_bugstracker.tpl'
	));
...
else //Show List of elements
{
	...
	$row = $Sql->query("SELECT COUNT(1) AS NB
		FROM ".PREFIX."bugstracker_parameters
		WHERE (nature=" . $nature .")",
		__LINE__, __FILE__);
	$nbr_elements = empty($row['NB']) ? 0 : $row['NB'];
